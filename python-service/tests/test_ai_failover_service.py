...
from app.services.ai_failover.model_config import (
    ModelProvider, ModelTier, ModelConfig, ModelStatus
)
...
def sample_messages():
    """샘플 메시지"""
    return [
        {"role": "user", "content": "Hello, how can you help me with Excel?"}
    ]
...
class TestModelConfigManager:
    ...
    def test_tier_filtering(self, ai_failover_service):
        """티어별 필터링 테스트"""
        premium_models = ai_failover_service.get_available_models(tier=ModelTier.PREMIUM)
...
class TestExecutionEngine:
    ...
    def test_validate_messages_invalid(self, ai_failover_service):
        ...
        assert not ai_failover_service.execution_engine.validate_messages(invalid_messages)
    
    def test_estimate_tokens(self, ai_failover_service, sample_messages):
        """토큰 수 추정 테스트"""
        token_count = ai_failover_service.execution_engine.estimate_tokens(sample_messages)
...
class TestAIFailoverService:
    ...
    async def test_get_available_models_with_filters(self, ai_failover_service):
        ...
        # 함수 호출 지원 모델만 조회
        function_models = ai_failover_service.get_available_models(supports_function_calling=True)
        for model in function_models:
            assert model.supports_function_calling
    
    @patch('app.services.ai_failover.execution_engine.ExecutionEngine.execute_chat_completion')
    async def test_chat_completion_success(self, mock_execute, ai_failover_service, sample_messages):
        """채팅 완성 성공 테스트"""
        mock_execute.return_value = "Test response"
        
        result = await ai_failover_service.chat_completion_with_failover(sample_messages)
        assert result == "Test response"
        mock_execute.assert_called_once()
    
    @patch('app.services.ai_failover.execution_engine.ExecutionEngine.execute_chat_completion')
    async def test_chat_completion_failover(self, mock_execute, ai_failover_service, sample_messages):
        """채팅 완성 페일오버 테스트"""
    ...
    @patch('app.services.ai_failover.execution_engine.ExecutionEngine.execute_chat_completion')
    async def test_chat_completion_all_models_fail(self, mock_execute, ai_failover_service, sample_messages):
        """모든 모델 실패 테스트"""
    ...
    @patch('app.services.ai_failover.execution_engine.ExecutionEngine.test_model_health')
...
class TestAIFailoverIntegration:
    ...
    async def test_complete_workflow(self, ai_failover_service, sample_messages):
        ...
        with patch('app.services.ai_failover.execution_engine.ExecutionEngine.execute_chat_completion') as mock_execute:
    ...
    async def test_stress_test(self, ai_failover_service, sample_messages):
        ...
        with patch('app.services.ai_failover.execution_engine.ExecutionEngine.execute_chat_completion') as mock_execute:
            ...
            tasks = []
            for i in range(10):
                task = ai_failover_service.chat_completion_with_failover(sample_messages)
                tasks.append(task)
