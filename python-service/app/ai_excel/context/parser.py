...
class ContextParser:
    ...
    def _extract_time_context(self, text: str) -> Dict[str, Any]:
        ...
        if year_matches:
            time_context["specific_dates"].extend(
                [{"type": "year", "value": int(y)} for y in year_matches]
            )
        ...
        if month_matches:
            time_context["specific_dates"].extend(
                [{"type": "month", "value": int(m)} for m in month_matches]
            )
        ...
        if quarter_matches:
            ...
            time_context["specific_dates"].extend(
                [{"type": "quarter", "value": int(q)} for q in quarter_matches]
            )

        # Check for relative time
        relative_matches = re.findall(self.time_patterns["relative"], text)
        if relative_matches:
            time_context["relative_time"] = self._parse_relative_time(
                relative_matches[0]
            )
    ...
    def _enrich_context(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (
            context["domain"] in ["finance", "hr"]
            and not context["entities"]["companies"]
        ):
    ...
    def _calculate_confidence(self, context: Dict[str, Any]) -> float:
        ...
        if (
            context["time_context"]["specific_dates"]
            or context["time_context"]["relative_time"]
        ):
            score += 0.2
