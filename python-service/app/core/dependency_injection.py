...
class ServiceLocator:
    ...
    @classmethod
    def register_service(cls, service_type: Type[T], factory: Callable[[], T], singleton: bool = True) -> None:
        """서비스 등록"""
        instance = cls()
            ...
            def factory():
                ...
                for arg_name, arg_config in constructor_args.items():
                    if isinstance(arg_config, dict) and 'service_type' in arg_config:
                        # 다른 서비스 참조
                        dep_module = __import__(arg_config['module'], fromlist=[arg_config['service_type']])
