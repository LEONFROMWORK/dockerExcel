...
class PWAService:
    ...
    async def track_installation(self, install_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "success",
                "install_record": installation_record
            }
    ...
    async def send_push_notification(
        ...
        try:
            ...
            logger.info(f"푸시 알림 전송 완료: {notification_id} - 성공: {sent_count}, 실패: {failed_count}")
    ...
    def _get_notification_actions(self, notification_type: str) -> List[Dict[str, str]]:
        ...
        return action_maps.get(notification_type, [
            {"action": "dismiss", "title": "닫기"}
        ])
    ...
    def _calculate_cache_efficiency(self) -> float:
        ...
        total_requests = self.offline_cache_stats["cache_hits"] + self.offline_cache_stats["cache_misses"]
    ...
    def _get_push_statistics(self) -> Dict[str, Any]:
        ...
        total_sent = sum(sub["delivery_stats"]["sent"] for sub in self.push_subscriptions.values())
        total_delivered = sum(sub["delivery_stats"]["delivered"] for sub in self.push_subscriptions.values())
        total_failed = sum(sub["delivery_stats"]["failed"] for sub in self.push_subscriptions.values())
        ...
        return {
            ...
            "active_subscriptions": len([
                sub for sub in self.push_subscriptions.values()
                if datetime.fromisoformat(sub["last_used"]) > datetime.now() - timedelta(days=30)
            ])
        }
