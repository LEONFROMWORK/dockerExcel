...
ws.cell(row=summary_row, column=4, value="=SUM(D2:D51)").font = Font(bold=True)
ws.cell(row=summary_row, column=6, value="=SUM(F2:F51)").font = Font(bold=True)
ws.cell(row=summary_row, column=6).number_format = "₩#,##0"
ws.cell(row=summary_row, column=8, value="=SUM(H2:H51)").font = Font(bold=True)
...
for idx, category in enumerate(categories, 4):
    ...
    ws2.cell(
        row=idx, column=2, value=f"=SUMIF('판매 데이터'!C:C,A{idx},'판매 데이터'!D:D)"
    )
    ws2.cell(
        row=idx, column=3, value=f"=SUMIF('판매 데이터'!C:C,A{idx},'판매 데이터'!H:H)"
    )
