...
from app.core.interfaces import IWebSocketHandler
...
class ExcelWebSocketHandler(IWebSocketHandler):
    ...
    async def handle_check_cell(self, session_id: str, data: Dict[str, Any]):
        ...
        try:
            ...
            if error:
                ...
                await self.notifier.notify_error_detected(
                    session_id, {"cell": cell, "sheet": sheet, "error": error_info}
                )
            else:
                # 오류 없음 알림
                await self.broadcast(
                    session_id,
                    {
                        ...
                        "data": {"cell": cell, "sheet": sheet, "has_error": False},
                    },
                )
    ...
    async def handle_fix_error(self, session_id: str, data: Dict[str, Any]):
        ...
        try:
            ...
            if not error_id:
                await self.send_error(session_id, "error_id가 필요합니다")
                return

            # Progress reporter 생성
            progress_reporter = WebSocketProgressReporter(session_id)

            # 캐시에서 오류 정보 가져오기
            error = await unified_cache.get_error_by_id(error_id)
            if not error:
                await self.send_error(
                    session_id, f"오류를 찾을 수 없습니다: {error_id}"
                )
                return

            # IntegratedErrorFixer 생성
            fixer = IntegratedErrorFixer(progress_reporter)

            # 수정 작업 시작 알림
            await progress_reporter.start_task("오류 수정", 1)

            # 오류 수정
            result = await fixer.fix(error)

            # 수정 결과 캐시
            await unified_cache.set_fix_result(error_id, result)

            if result.success:
                # 성공 알림
                await self.notifier.notify_error_fixed(
                    session_id,
                    {
                        "error_id": error_id,
                        "status": "success",
                        ...
                        "message": result.message or "오류가 성공적으로 수정되었습니다",
                    },
                )

                # auto_apply가 true이고 파일 ID가 있으면 실제 적용
                if auto_apply and self.active_sessions[session_id].get("file_id"):
                    file_id = self.active_sessions[session_id]["file_id"]
                    # 실제 파일에 수정 사항 적용
                    from app.core.file_path_resolver import FilePathResolver

                    file_path = await FilePathResolver.get_file_path(file_id)
                    ...
                    if file_path:
                        try:
                            ...
                            worksheet = workbook[result.error.sheet]
                            cell = worksheet[result.error.cell]

                            # 수정된 값 적용
                            if hasattr(result, "fixed_value"):
                                cell.value = result.fixed_value
                            elif hasattr(result, "fixed_formula"):
                                cell.value = result.fixed_formula

                            # 파일 저장
                            workbook.save(file_path)
                            workbook.close()

                            logger.info(f"Successfully applied fix to file {file_id}")

                            # 캐시 무효화
                            from app.core.integrated_cache import integrated_cache

                            await integrated_cache.delete(f"analysis:{file_id}")
                            await integrated_cache.delete(f"errors:{file_id}")
                        except Exception as e:
                            logger.error(f"Failed to apply fix to file: {str(e)}")
            else:
                # 실패 알림
                await self.notifier.notify_error_fixed(
                    session_id,
                    {
                        "error_id": error_id,
                        "status": "failed",
                        "message": result.message or "오류 수정에 실패했습니다",
                    },
                )

            # 작업 완료 알림
            await progress_reporter.complete_task(
                "오류 수정",
                {
            ...
            logger.error(f"오류 수정 처리 실패: {str(e)}", exc_info=True)
            await self.send_error(
                session_id, f"오류 수정 중 문제가 발생했습니다: {str(e)}"
            )
    ...
    async def handle_analyze_file(self, session_id: str, data: Dict[str, Any]):
        ...
        try:
            ...
            # 세션에 파일 ID 저장
            if file_id and session_id in self.active_sessions:
                self.active_sessions[session_id]["file_id"] = file_id
            ...
            # 결과를 통합 캐시에 저장
            if result.get("status") == "success" and file_id:
                await unified_cache.set_analysis(file_id, result)
                ...
                if result.get("errors"):
                    ...
                    errors = [
                        ExcelError(**error_dict) for error_dict in result["errors"]
                    ]
                    await unified_cache.set_errors(file_id, errors)
    ...
    async def handle_update_context(self, session_id: str, data: Dict[str, Any]):
        """컨텍스트 업데이트 처리 - 현재 미사용"""
        # TODO: 필요시 구현
        await self.send_error(session_id, "컨텍스트 업데이트는 현재 지원되지 않습니다")
    ...
    async def handle_ai_query(self, session_id: str, data: Dict[str, Any]):
        ...
        try:
            ...
            # 임시 응답
            await self.notifier.notify_ai_suggestion(
                session_id,
                {
                    "query": query,
    ...
    async def send_error(self, session_id: str, error_message: str):
        ...
        error_response = ResponseBuilder.error(
            message=error_message, error_code="WEBSOCKET_ERROR"
        )
    ...
    def get_active_sessions(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            session_id: {
                ...
                "context": (
                    info["context"].__dict__
                    if hasattr(info["context"], "__dict__")
                    else {}
                ),
            }
