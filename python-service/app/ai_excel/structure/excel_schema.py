...
class FormulaDefinition(BaseModel):
    ...
    cell_reference: str = Field(..., description="Cell or range reference (e.g., 'E2:E10')")
    formula_type: FormulaType
    formula: str = Field(..., description="Excel formula string")
    description: Optional[str] = None
    depends_on: Optional[List[str]] = Field(default_factory=list, description="Cell references this formula depends on")
...
class DataRelationship(BaseModel):
    ...
    target_sheet: str
    target_range: str
    relationship_type: str = Field(..., description="Type of relationship (e.g., 'lookup', 'sum', 'reference')")
...
class ConditionalFormat(BaseModel):
    ...
    rule_type: str = Field(..., description="Type of rule (e.g., 'cell_value', 'color_scale', 'data_bar')")
...
class SheetSchema(BaseModel):
    ...
    freeze_panes: Optional[str] = Field(None, description="Cell reference for freeze panes (e.g., 'B2')")
...
class GenerationResponse(BaseModel):
    ...
    insights: Optional[List[str]] = Field(default_factory=list, description="AI insights about the generation")
    warnings: Optional[List[str]] = Field(default_factory=list, description="Any warnings during generation")
