...
5. 한국어로 친절하게 응답"""
    ...
    async def process(self, query: str, context: Context) -> Dict[str, Any]:
        ...
        try:
            ...
            raise AIServiceError(
                f"AI 처리 중 오류 발생: {str(e)}", code="AI_PROCESSING_ERROR"
            )
    ...
    async def get_fix_suggestion(
        self, error: ExcelError, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """특정 오류에 대한 수정 제안"""
        try:
    ...
    def _analyze_query_type(self, query: str) -> str:
        ...
        if any(keyword in query_lower for keyword in ["수정", "고치", "fix", "해결"]):
            ...
            keyword in query_lower for keyword in ["도움", "설명", "help", "방법"]
        ):
            return "general_help"
        else:
            return "general"

    async def _handle_error_fix_query(
...
"""
        ...
        response = await self.openai_service.get_completion(
            prompt=prompt, system_prompt=self.system_prompt, temperature=0.5
        )
    ...
    async def _handle_error_analysis_query(
        self, query: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """오류 분석 관련 쿼리 처리"""
...
"""
    ...
    async def _handle_general_help_query(
        self, query: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """일반 도움말 쿼리 처리"""
        prompt = f"""
...
"""
        ...
        return {"response": response, "suggestions": [], "error_references": []}
