...
from app.services.analysis import FormulaAnalyzer
...
async def analyze_excel_formulas(
    ...
    try:
        ...
        response_data = {
            ...
            "summary": {
                ...
                "complexity_distribution": {
                    k.value: v
                    for k, v in analysis_result.complexity_distribution.items()
                },
                "most_used_functions": [
        ...
        if (
            ...
            and analysis_result.dependency_graph.number_of_nodes() > 0
        ):
        ...
        raise HTTPException(
            status_code=500, detail=f"수식 분석 중 오류가 발생했습니다: {str(e)}"
        )
    finally:
        # 임시 파일 정리
    ...
    try:
        ...
        return {
            ...
            "analysis": {
                ...
                "function_categories": [
                    cat.value for cat in analysis.function_categories
                ],
...
async def optimize_formulas(
    ...
    target_areas: Optional[str] = Query(
        None, description="최적화 대상 영역 (쉼표 구분)"
    ),
) -> Dict[str, Any]:
    """
    수식 최적화 제안 및 적용
    ...
    try:
        ...
        volatile_optimizations = _generate_volatile_optimizations(
            analysis_result, optimization_level
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"수식 최적화 중 오류가 발생했습니다: {str(e)}"
        )
    finally:
        if os.path.exists(tmp_path):
            os.unlink(tmp_path)
...
async def analyze_formula_dependencies(
    ...
    try:
        ...
        dependency_info = {
            "total_nodes": analysis_result.dependency_graph.number_of_nodes(),
            "total_edges": analysis_result.dependency_graph.number_of_edges(),
            "is_acyclic": nx.is_directed_acyclic_graph(
        ...
        if target_cell:
            if target_cell in analysis_result.dependency_graph:
                # 선행 셀 (이 셀이 의존하는 셀들)
                predecessors = list(
                    analysis_result.dependency_graph.predecessors(target_cell)
                )

                # 후행 셀 (이 셀에 의존하는 셀들)
                successors = list(
                    analysis_result.dependency_graph.successors(target_cell)
                )
                ...
                dependency_info["target_cell"] = {
                    ...
                    "dependency_chain_length": len(
                        nx.ancestors(analysis_result.dependency_graph, target_cell)
                    ),
                }
        ...
        try:
            ...
            dependency_info["circular_references"] = [
                {"cycle": cycle, "length": len(cycle)} for cycle in cycles[:5]
            ]
        ...
        raise HTTPException(
            status_code=500, detail=f"의존성 분석 중 오류가 발생했습니다: {str(e)}"
        )
    finally:
        if os.path.exists(tmp_path):
            os.unlink(tmp_path)
...
def _generate_volatile_optimizations(
    analysis_result, level: str
) -> List[Dict[str, Any]]:
    """휘발성 함수 최적화 제안 생성"""
    optimizations = []
...
def _calculate_improvement_estimate(
