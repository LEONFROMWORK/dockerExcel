...
class ExcelToHtmlConverter:
    ...
    def convert_file_to_html(self, file_path: str, sheet_name: str = None) -> Dict[str, Any]:
    ...
    def convert_sheet_to_html(self, worksheet) -> Dict[str, Any]:
        ...
        for row in range(1, max_row + 1):
            ...
            for col in range(1, max_col + 1):
                ...
                cell_attrs = {
                    'class': 'excel-cell',
                    'style': self._get_cell_style(cell, col_widths[col], row_heights[row])
                }
                
                # Check if this is a merged cell master
                for merge in merged_cells:
                    if merge['start']['row'] == row - 1 and merge['start']['col'] == col - 1:
                        ...
                        total_width = sum(col_widths[c] for c in range(col, col + merge['colspan']))
                        total_height = sum(row_heights[r] for r in range(row, row + merge['rowspan']))
                        
                        cell_attrs['style'] += f' width: {total_width}px; height: {total_height}px;'
                        break
