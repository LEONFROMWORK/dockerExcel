...
async def get_metric_history(
    metric_name: str,
    minutes: int = Query(60, description="조회할 기간 (분)")
) -> Dict[str, Any]:
    """특정 메트릭의 히스토리 조회"""
    try:
        if minutes > 1440:  # 최대 24시간
            raise HTTPException(status_code=400, detail="최대 1440분(24시간)까지 조회 가능합니다")
...
async def get_alerts(
    ...
    try:
        if active_only:
            alerts = [alert.to_dict() for alert in monitoring_service.active_alerts.values()]
            total_count = len(alerts)
        else:
            # 최근 50개 알림
            alerts = [alert.to_dict() for alert in list(monitoring_service.alert_history)[-50:]]
...
async def get_job_progress(job_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if progress:
            ...
            raise HTTPException(status_code=404, detail="작업 진행률 정보를 찾을 수 없습니다")
...
async def update_job_progress(request: JobProgressUpdateRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"작업 진행률 업데이트 실패: {str(e)}")
...
async def get_performance_summary(
    ...
    try:
        if hours > 24:
            raise HTTPException(status_code=400, detail="최대 24시간까지 조회 가능합니다")
        ...
        return {
            ...
            "data": {
                ...
                "total_metrics_collected": sum(
                    len(metrics) for metrics in monitoring_service.metrics_history.values()
                ),
