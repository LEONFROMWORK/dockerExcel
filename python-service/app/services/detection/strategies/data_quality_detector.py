...
class DataQualityDetector(IErrorDetector):
    ...
    def _detect_duplicates(self, worksheet: Any, sheet_name: str) -> List[ExcelError]:
        ...
        for value, positions in value_positions.items():
            if len(positions) > 1:
                ...
                for row, col, coord in positions[1:]:
                    error = ExcelError(
                        ...
                        formula=None,
                        value=value,
                        message=f"중복된 값: '{value}'가 {positions[0][2]}에도 있습니다",
                        severity="medium",
                        is_auto_fixable=False,
                        suggested_fix="중복된 값을 확인하고 필요시 제거하세요",
                        confidence=0.9
                    )
                    errors.append(error)
                    ...
                    for item in values:
                        if item['value'] is None or item['value'] == "":
                            error = ExcelError(
                                ...
                                severity="low" if fill_rate > 0.8 else "medium",
                                is_auto_fixable=False,
                                suggested_fix="누락된 데이터를 입력하세요",
                                confidence=0.8
                            )
                            errors.append(error)
    ...
    def _detect_type_mismatches(self, worksheet: Any, sheet_name: str) -> List[ExcelError]:
        """타입 불일치 감지"""
        errors = []
                    ...
                    if cell.value is not None and cell.value != "":
                        ...
                        if actual_type != expected_type:
                            error = ExcelError(
                                ...
                                severity="medium",
                                is_auto_fixable=True if actual_type == "text" and expected_type == "number" else False,
