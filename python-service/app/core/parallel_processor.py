...
class ParallelProcessor:
    ...
    async def process_batch_async(
        ...
        if mode == ProcessingMode.SEQUENTIAL:
            ...
            results = await self._process_async_parallel(
                items, processor_func, chunk_size
            )

        total_time = time.time() - start_time
        logger.info(
            f"배치 처리 완료: {len(items)}개 항목, {total_time:.2f}초, 모드: {mode.value}"
        )
    ...
    async def _process_sequential_async(
        ...
        for item in items:
            ...
            try:
                ...
                results.append(
                    ProcessingResult(
                        success=True, result=result, processing_time=time.time() - start
                    )
                )
        ...
        async def process_item(item):
            ...
            try:
                result = await loop.run_in_executor(
                    self.thread_executor, processor_func, item
                )
    ...
    async def _process_process_parallel(
        ...
        if hasattr(processor_func, "__name__"):
            ...
            logger.warning(
                "프로세스 병렬 처리는 피클 가능한 함수만 지원합니다. 스레드 병렬로 전환합니다."
            )
        ...
        for chunk in chunks:
            chunk_results = await loop.run_in_executor(
                self.process_executor, self._process_chunk_sync, chunk, processor_func
            )
    ...
    def _process_chunk_sync(
        chunk: List[Any], processor_func: Callable
    ) -> List[ProcessingResult]:
        """동기 청크 처리 (프로세스 실행용)"""
        results = []
    ...
    async def _process_async_parallel(
        self, items: List[Any], processor_func: Callable, chunk_size: int
    ) -> List[ProcessingResult]:
        """비동기 병렬 처리"""
    ...
    def process_excel_files_parallel(
        ...
        async def process_files():
            results = await self.process_batch_async(file_paths, analysis_func, mode)
...
class ExcelBatchProcessor:
    ...
    async def batch_analyze_errors(
        self, file_paths: List[str], error_detector: Any
    ) -> Dict[str, Any]:
        """배치 오류 분석"""
        ...
        results = await self.processor.process_batch_async(
            file_paths, analyze_file, ProcessingMode.ASYNC_PARALLEL, chunk_size=5
        )

        return self._aggregate_results(file_paths, results)

    async def batch_compare_files(
        self, file_pairs: List[tuple], comparison_engine: Any
    ) -> Dict[str, Any]:
        """배치 파일 비교"""

        async def compare_pair(pair: tuple):
            expected_file, actual_file = pair
            return await comparison_engine.compare_files(expected_file, actual_file)

        results = await self.processor.process_batch_async(
            file_pairs, compare_pair, ProcessingMode.THREAD_PARALLEL
        )

        return self._aggregate_comparison_results(file_pairs, results)

    async def batch_optimize_formulas(
        self, file_paths: List[str], formula_analyzer: Any
    ) -> Dict[str, Any]:
        """배치 수식 최적화"""
        ...
        results = await self.processor.process_batch_async(
            file_paths, analyze_formulas, ProcessingMode.ASYNC_PARALLEL, chunk_size=3
        )

        return self._aggregate_formula_results(file_paths, results)

    def _aggregate_results(
        self, file_paths: List[str], results: List[ProcessingResult]
    ) -> Dict[str, Any]:
        """결과 집계"""
        ...
        return {
            "summary": {
                ...
                "avg_processing_time": (
                    total_processing_time / len(results) if results else 0
                ),
            },
            "details": [
                {
    ...
    def _aggregate_comparison_results(
        self, file_pairs: List[tuple], results: List[ProcessingResult]
    ) -> Dict[str, Any]:
        """비교 결과 집계"""
            ...
            "comparisons": [
                {
                    ...
                    "success": result.success,
                    "differences": (
                        result.result.differences_found if result.success else None
                    ),
                    "match_percentage": (
                        result.result.match_percentage if result.success else None
                    ),
                    "error": result.error,
                }
        ...
        for result in results:
            if result.success and result.result:
                ...
                for level, count in result.result.complexity_distribution.items():
                    ...
                    complexity_distribution[key] = (
                        complexity_distribution.get(key, 0) + count
                    )

                # 최적화 기회 수집
                optimization_opportunities.extend(
                    result.result.optimization_opportunities
                )
            ...
            "file_results": [
                {
                    ...
                    "success": result.success,
                    "formula_count": (
                        result.result.total_formulas if result.success else 0
                    ),
                    "error": result.error,
                }
                for file_path, result in zip(file_paths, results)
