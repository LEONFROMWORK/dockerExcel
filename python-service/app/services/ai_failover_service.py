...
from .ai_failover.model_config import (
    ModelProvider, ModelTier, ModelConfig, ModelStatus, ModelConfigManager
)
...
class AIFailoverService:
    ...
    def get_available_models(
        ...
        for model in self.config_manager.get_all_models():
            ...
            # 필터 적용
            if self._matches_filters(model, tier, supports_vision, supports_function_calling):
    ...
    def _matches_filters(self, model, tier, supports_vision, supports_function_calling):
        ...
        if supports_vision is not None and model.supports_vision != supports_vision:
            return False
        if supports_function_calling is not None and model.supports_function_calling != supports_function_calling:
            return False
        return True
    ...
    async def _try_model_execution(self, model_config, messages, temperature, max_tokens, stream):
        """개별 모델 실행 시도"""
        logger.info(f"Attempting with model: {model_config.provider.value}:{model_config.model_name}")
    ...
    def _update_model_failure(self, model_config: ModelConfig, error_message: str):
        ...
        if status.consecutive_failures >= 3:
            ...
            logger.warning(f"Model {model_key} marked as unhealthy after 3 consecutive failures")
    ...
    def _start_health_monitoring(self):
        ...
        try:
            ...
            logger.debug("Event loop not running, health monitoring will be started later")
            pass
