...
class StructureDetector(IErrorDetector):
    ...
    def _detect_empty_rows_cols(
        ...
        for col_idx in range(worksheet.max_column, 0, -1):
            has_data = any(
                worksheet.cell(row=row, column=col_idx).value is not None
                for row in range(1, min(worksheet.max_row + 1, 100))
            )
            if has_data:
                data_end_col = col_idx
                break
        ...
        for col_idx in range(data_start_col, data_end_col):
            col_empty = all(
                ...
                for row in range(
                    1, min(worksheet.max_row + 1, 100)
                )  # 처음 100행만 확인
            )

            # 데이터가 있는 열들 사이의 빈 열만 오류로 표시
            if col_empty:
                # 앞뒤에 데이터가 있는지 확인
                has_data_before = (
                    any(
                        worksheet.cell(row=row, column=col_idx - 1).value is not None
                ...
                if has_data_before and has_data_after:
                    ...
                    error = ExcelError(
                        ...
                        formula=None,
                        value=None,
                        message=f"데이터 사이에 빈 열 발견: {col_letter}열",
                        severity="low",
                        is_auto_fixable=True,
                        suggested_fix="데이터 구조 개선을 위해 빈 열 제거를 고려하세요",
                        confidence=0.7,
                    )
                    errors.append(error)
    ...
    def _detect_table_structure_issues(
        ...
        for table in tables:
            ...
            # 빈 헤더 검사 (테이블 크기가 큰 경우에만)
            empty_headers = [
                i for i, h in enumerate(headers) if h is None or str(h).strip() == ""
            ]
            if empty_headers and len(headers) >= 5:  # 열이 5개 이상인 테이블에서만
                # 모든 헤더가 비어있으면 테이블이 아닐 가능성이 높음
                non_empty_headers = [
                    h for h in headers if h is not None and str(h).strip() != ""
                ]
                if len(non_empty_headers) >= 2:  # 최소 2개 이상의 헤더가 있는 경우
                    error = ExcelError(
                        ...
                        formula=None,
                        value=None,
                        message="테이블 헤더에 빈 값이 있습니다",
                        severity="low",
                        is_auto_fixable=False,
                        suggested_fix="필요한 경우 모든 열에 헤더를 지정하세요",
                        confidence=0.7,
                    )
                    errors.append(error)
    ...
    def _detect_inconsistent_formats(
        self, worksheet: Any, sheet_name: str
    ) -> List[ExcelError]:
        """일관성 없는 형식 감지"""
        errors = []
        ...
        for col_idx, formats in column_formats.items():
            if len(formats) > 1:
                ...
                # 주요 형식이 50% 미만인 경우만 오류로 표시
                # 그리고 서로 다른 형식이 3개 이상일 때만
                if main_format[1] / total_cells < 0.5 and len(formats) >= 3:
                    col_letter = self._get_column_letter(col_idx)
                    error = ExcelError(
                        ...
                        severity="low",
                        is_auto_fixable=True,
                        suggested_fix="데이터 성격에 맞게 형식을 통일하면 더 좋습니다",
                        confidence=0.6,
                    )
                    errors.append(error)
        ...
        for row in range(1, min(worksheet.max_row + 1, 1000)):
            for col in range(1, min(worksheet.max_column + 1, 100)):
                if (row, col) not in visited:
                    ...
                    if cell.value is not None:
                        # 연결된 데이터 영역 찾기
                        table = self._find_connected_region(
                            worksheet, row, col, visited
                        )

                        # 최소 크기 이상인 경우만 테이블로 간주
                        if (
                            table["end_row"] - table["start_row"]
                            >= self.min_table_height - 1
                            and table["end_col"] - table["start_col"]
                            >= self.min_table_width - 1
                        ):
                            table["id"] = table_id
                            tables.append(table)
                            table_id += 1
    ...
    def _find_connected_region(
        self, worksheet: Any, start_row: int, start_col: int, visited: Set[tuple]
    ) -> Dict[str, Any]:
        """연결된 데이터 영역 찾기"""
        ...
        while queue:
            ...
            for dr, dc in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
                ...
                if (new_row, new_col) not in visited:
                    if (
                        1 <= new_row <= worksheet.max_row
                        and 1 <= new_col <= worksheet.max_column
                    ):
