...
try:
    ...
    logging.warning("formulas library not available. Install with: pip install formulas")
...
class RealtimeFormulaValidator:
    ...
    def __init__(self):
        ...
        if not FORMULAS_AVAILABLE:
            raise ImportError("formulas library is required. Install with: pip install formulas")
    ...
    def validate_formula_realtime(self, 
                                formula: str, 
                                cell_address: str, 
                                sheet_name: str = 'Sheet1') -> ValidationResult:
        """실시간 수식 검증"""
    ...
    def batch_validate_changes(self, changes: List[Dict]) -> Dict[str, ValidationResult]:
        """변경사항 일괄 검증"""
        results = {}
        ...
        for change in changes:
            ...
            if formula:
                temp_context[sheet][cell] = f"={formula}" if not formula.startswith('=') else formula
        ...
        try:
            ...
            for change in changes:
                ...
                if formula:
                    results[change_id] = self.validate_formula_realtime(formula, cell, sheet)
        
        return results
    
    def simulate_error_conditions(self, formula: str, sheet_name: str = 'Sheet1') -> List[Dict]:
        """오류 조건 시뮬레이션"""
        errors = []
        ...
        try:
            ...
            error_scenarios = [
                # Division by zero
                {'type': 'division_by_zero', 'test': lambda f: '/0' in f or 'B2' in f and self.context.get(sheet_name, {}).get('B2') == 0},
                # Circular reference
                {'type': 'circular_reference', 'test': lambda f: self._check_circular_reference(f, sheet_name)},
                # Invalid reference
                {'type': 'invalid_reference', 'test': lambda f: self._check_invalid_references(f, sheet_name)},
                # Type mismatch
                {'type': 'type_mismatch', 'test': lambda f: self._check_type_mismatch(f, sheet_name)}
            ]
            ...
            for scenario in error_scenarios:
                if scenario['test'](formula):
                    errors.append({
                        'type': scenario['type'],
                        'severity': 'high' if scenario['type'] in ['circular_reference', 'division_by_zero'] else 'medium',
                        'description': self._get_error_description(scenario['type']),
    ...
    def _classify_formula_type(self, formula: str) -> str:
        ...
        if any(func in formula_upper for func in ['VLOOKUP', 'HLOOKUP', 'INDEX', 'MATCH', 'XLOOKUP']):
            return 'lookup'
        elif any(func in formula_upper for func in ['SUM', 'AVERAGE', 'COUNT', 'MIN', 'MAX']):
            return 'aggregation'
        elif 'IF' in formula_upper:
            return 'conditional'
        elif any(func in formula_upper for func in ['LEFT', 'RIGHT', 'MID', 'CONCATENATE', 'TEXT']):
            return 'text'
    ...
    def _generate_warnings(self, formula: str, result: Any) -> List[str]:
        ...
        if any(func in formula.upper() for func in volatile_functions):
            warnings.append("휘발성 함수가 포함되어 있습니다. 파일이 열릴 때마다 재계산됩니다.")
