...
from ..models.template_metadata import (
    EnhancedTemplateMetadata, TemplateCategory, TemplateComplexity, 
    UsageContext, SectionDefinition, FieldDefinition, WorkflowStep
)
...
class TemplateAutoProcessor:
    ...
    async def process_crawled_templates(self, crawled_templates: List[CrawledTemplate]) -> Dict[str, Any]:
        ...
        for i, crawled_template in enumerate(crawled_templates):
            logger.info(f"📄 템플릿 처리 중 ({i+1}/{len(crawled_templates)}): {crawled_template.title}")
            ...
            try:
                ...
                # 2. 메타데이터 생성
                metadata = await self._generate_enhanced_metadata(crawled_template, analysis_result)
        ...
        # 처리 시간 계산
        processing_summary["processing_time"] = str(datetime.now() - processing_summary["start_time"])
        processing_summary["success_rate"] = round(
            (processing_summary["successfully_processed"] / processing_summary["total_templates"]) * 100, 2
        ) if processing_summary["total_templates"] > 0 else 0
        
        logger.info(f"✅ 템플릿 자동 처리 완료: {processing_summary['successfully_processed']}/{processing_summary['total_templates']}")
        
        return processing_summary
    
    async def _analyze_template_file(self, crawled_template: CrawledTemplate) -> Optional[Dict[str, Any]]:
        """템플릿 파일 분석"""
        try:
            # 로컬 파일 경로 확인
            if not hasattr(crawled_template, 'local_path') or not crawled_template.local_path:
    ...
    async def _generate_enhanced_metadata(
        ...
        try:
            ...
            # 사용 맥락 추론
            usage_contexts = self._infer_usage_contexts(template_category, crawled_template.description)
            ...
            metadata = EnhancedTemplateMetadata(
                ...
                business_use_cases=self._extract_use_cases(crawled_template.description),
                target_audience=self._infer_target_audience(template_category, crawled_template.description),
                usage_context=usage_contexts,
                complexity=complexity,
                estimated_completion_time=self._estimate_completion_time(complexity, analysis_result),
                prerequisites=self._generate_prerequisites(template_category),
                context_keywords=self._extract_keywords(crawled_template.title, crawled_template.description),
                semantic_tags=self._generate_semantic_tags(template_category, crawled_template.description),
    ...
    def _determine_template_category(
        ...
        if major_category in self.category_mapping:
            for keyword, category in self.category_mapping[major_category].items():
                if keyword in minor_category or keyword in title or keyword in description:
                    return category
        ...
        # 재무 관련
        if any(keyword in text_to_search for keyword in ['재무', '회계', '현금', '수익', '매출', '손익', '예산']):
            return TemplateCategory.FINANCE
        
        # 인사 관련
        if any(keyword in text_to_search for keyword in ['인사', '급여', '근태', '직원', '사원']):
            return TemplateCategory.HR
        
        # 영업 관련
        if any(keyword in text_to_search for keyword in ['영업', '고객', '판매', '마케팅']):
            return TemplateCategory.SALES
        
        # 운영 관련
        if any(keyword in text_to_search for keyword in ['운영', '관리', '프로젝트', '일정']):
    ...
    def _determine_complexity(
        ...
        # 분석 결과 기반 복잡도 판단
        if analysis_result:
            complexity_score = analysis_result.get('overall_structure', {}).get('complexity_score', 1)
    ...
    def _infer_usage_contexts(self, category: TemplateCategory, description: str) -> List[UsageContext]:
        """사용 맥락 추론"""
        contexts = []
        
        if category == TemplateCategory.FINANCE:
            contexts.extend([UsageContext.MONTHLY_REPORTS, UsageContext.QUARTERLY_ANALYSIS])
        
        if category == TemplateCategory.HR:
            contexts.append(UsageContext.MONTHLY_REPORTS)
        
        if category == TemplateCategory.SALES:
            contexts.extend([UsageContext.DAILY_OPERATIONS, UsageContext.MONTHLY_REPORTS])
    ...
    def _generate_purpose(self, crawled_template: CrawledTemplate) -> str:
        ...
        for keyword, purpose in purpose_templates.items():
            if keyword in crawled_template.title or keyword in crawled_template.description:
                return purpose
        
        # 기본 목적
        return f"{crawled_template.title}을 통해 업무 효율성을 높이고 체계적인 관리를 지원"
    ...
    def _infer_target_audience(self, category: TemplateCategory, description: str) -> List[str]:
        """대상 사용자 추론"""
        audiences = []
        ...
        category_audiences = {
            ...
            TemplateCategory.SALES: ["영업담당자", "영업관리자", "마케팅팀", "사업개발팀"],
    ...
    def _estimate_completion_time(
        ...
        # 분석 결과가 있으면 더 정확한 추정
        if analysis_result:
            sheet_count = analysis_result.get('overall_structure', {}).get('total_sheets', 1)
            if sheet_count > 3:
    ...
    def _generate_prerequisites(self, category: TemplateCategory) -> List[str]:
        ...
        prerequisites_map = {
            TemplateCategory.FINANCE: [
                "과거 재무 데이터", "회계 기본 지식", "관련 문서 및 자료"
            ],
            TemplateCategory.HR: [
                "직원 기본 정보", "급여 및 복리후생 정책", "조직도"
            ],
            TemplateCategory.SALES: [
                "고객 정보", "제품/서비스 목록", "영업 프로세스 이해"
            ],
            TemplateCategory.OPERATIONS: [
                "업무 프로세스 이해", "관련 데이터", "조직 구조 파악"
            ],
            TemplateCategory.INVENTORY: [
                "제품 목록", "공급업체 정보", "창고 현황"
            ],
            TemplateCategory.PROJECT_MANAGEMENT: [
                "프로젝트 범위 정의", "팀원 정보", "일정 계획"
            ]
        }
        
        return prerequisites_map.get(category, ["관련 데이터", "기본적인 엑셀 사용 능력"])
    ...
    def _extract_keywords(self, title: str, description: str) -> List[str]:
        ...
        # 일반적인 업무 키워드
        business_keywords = [
            "관리", "분석", "계획", "보고", "모니터링", "평가", "개선",
            "효율", "성과", "품질", "비용", "수익", "매출", "예산"
        ]
    ...
    def _generate_semantic_tags(self, category: TemplateCategory, description: str) -> List[str]:
        """의미적 태그 생성"""
        base_tags = {
            ...
            TemplateCategory.PROJECT_MANAGEMENT: ["프로젝트관리", "일정관리", "자원배분"]
        }
    ...
    async def _process_i18n(
        ...
        """I18n 처리"""
        try:
            i18n_requirements = await i18n_template_service.analyze_template_i18n_requirements(analysis_result)
    ...
    async def _generate_ai_context(self, metadata: EnhancedTemplateMetadata) -> Optional[Dict[str, Any]]:
        """AI 컨텍스트 생성"""
        try:
