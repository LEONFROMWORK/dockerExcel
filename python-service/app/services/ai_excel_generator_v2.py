...
class AIExcelGeneratorV2:
    ...
    async def get_generation_insights(
        self, user_request: str, language: str = "ko"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            # Build context
            context = await self.coordinator.context_builder.build(
                user_request, {"language": language}
            )
            ...
            insights = {
                ...
                "recommended_approach": analysis.get("generation_strategy", {}).get(
                    "approach"
                ),
                ...
                "confidence_score": analysis.get("ai_insights", {}).get(
                    "confidence", 0.5
                ),
            }
        ...
        if result.get("status") == "success":
            ...
            return {
                ...
                "metadata": {
                    ...
                    "complexity": structure.get("generation_hints", {}).get(
                        "complexity"
                    ),
                },
            }
...
class AIExcelGenerator:
    ...
    async def generate_from_natural_language(self, *args, **kwargs):
        return await ai_excel_generator_v2.generate_from_natural_language(
            *args, **kwargs
        )

    async def find_or_generate_template(self, request: str, language: str = "ko"):
        """Compatibility method"""
        insights = await ai_excel_generator_v2.get_generation_insights(
            request, language
        )
