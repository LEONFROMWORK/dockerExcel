...
class InteractiveExcelGenerator:
    ...
    async def start_generation_session(
        self, user_id: str, initial_request: str, language: str = "ko"
    ) -> Dict[str, Any]:
        """생성 세션 시작"""
        ...
        # 초기 의도 파악 및 질문 생성
        clarification_needed = await self._analyze_and_clarify(
            session_id, initial_request
        )
    ...
    async def process_user_response(
        ...
        self.conversation_history[session_id].append(
            {"role": "user", "content": user_response, "timestamp": datetime.now()}
        )
    ...
    async def _enhance_understanding(self, session_id: str) -> Dict[str, Any]:
        """이해도 향상"""

        session = self.sessions[session_id]
        history = self.conversation_history[session_id]

        system_prompt = """Based on the conversation history, create a comprehensive understanding
    ...
    async def _propose_structure(self, session_id: str) -> Dict[str, Any]:
        """Excel 구조 제안"""
        ...
        structure = await ai_excel_generator._design_excel_structure(
            understanding, session["language"]
        )
        ...
        simplified_structure = {
            ...
            "includes_charts": any(
                sheet.get("charts") for sheet in structure.get("sheets", [])
            ),
    ...
    async def _modify_structure(self, session_id: str, feedback: str) -> Dict[str, Any]:
        """구조 수정"""
    ...
    async def _request_data_requirements(self, session_id: str) -> Dict[str, Any]:
        """데이터 요구사항 요청"""
        ...
        for sheet in structure.get("sheets", []):
            if any(col.get("type") == "number" for col in sheet.get("columns", [])):
                questions.append(
                    "숫자 데이터의 범위는 어느 정도인가요? (예: 0-100, 1000-10000)"
                )

            if sheet.get("includes_dates"):
                questions.append(
                    "날짜 범위는 어떻게 설정할까요? (예: 최근 1년, 2024년)"
                )
    ...
    async def _generate_preview(self, session_id: str) -> Dict[str, Any]:
        """미리보기 생성"""
        ...
        for sheet in structure.get("sheets", [])[:2]:  # 처음 2개 시트만
            sheet_preview = {
                ...
                "sample_rows": [
                    [
                        f"데이터{i}_{j}"
                        for j in range(min(5, len(sheet.get("columns", []))))
                    ]
                    for i in range(3)
                ],
                ...
                "formulas": sum(
                    1 for col in sheet.get("columns", []) if col.get("formula")
                ),
            }
    ...
    async def _apply_refinements(self, session_id: str, refinement: str):
        """수정사항 적용"""
    ...
    async def _finalize_generation(self, session_id: str) -> Dict[str, Any]:
        """최종 생성"""
