...
async def analyze_existing_charts(file: UploadFile = File(...)):
    """기존 차트 분석"""
...
async def suggest_optimal_charts(
    ...
    sheet_name: Optional[str] = Query(
        None, description="분석할 시트명 (전체 시트 분석시 생략)"
    ),
):
    """데이터 기반 최적 차트 제안"""
    ...
    try:
        suggestions = excel_chart_analyzer.suggest_optimal_charts(
...
async def create_chart(file: UploadFile = File(...), chart_config: str = Form(...)):
    """지정된 설정으로 차트 생성"""
...
async def auto_generate_charts(
    ...
    try:
        result = excel_chart_analyzer.auto_generate_charts(
            tmp_file_path, max_charts_per_sheet
        )
...
async def analyze_existing_pivots(file: UploadFile = File(...)):
    """기존 피벗테이블 분석"""
...
async def suggest_optimal_pivots(
    ...
    try:
        suggestions = excel_pivot_analyzer.suggest_optimal_pivots(
            tmp_file_path, sheet_name
        )
...
async def create_pivot_table(
    file: UploadFile = File(...), pivot_config: str = Form(...)
):
    """지정된 설정으로 피벗테이블 생성"""
...
async def auto_generate_pivots(
    ...
    try:
        result = excel_pivot_analyzer.auto_generate_pivots(
            tmp_file_path, max_pivots_per_sheet
        )
...
async def create_cross_tabulation(
    file: UploadFile = File(...), crosstab_config: str = Form(...)
):
    """교차표 생성"""
...
async def comprehensive_analysis(
    ...
    try:
        ...
        if include_charts:
            ...
            comprehensive_result["analysis_summary"]["existing_charts"] = (
                chart_analysis.get("total_charts", 0)
            )
            ...
            if auto_generate_suggestions:
                ...
                comprehensive_result["analysis_summary"]["chart_suggestions"] = (
                    chart_suggestions.get("total_suggestions", 0)
                )
        ...
        if include_pivots:
            ...
            comprehensive_result["analysis_summary"]["existing_pivots"] = (
                pivot_analysis.get("total_pivots", 0)
            )
            ...
            if auto_generate_suggestions:
                ...
                comprehensive_result["analysis_summary"]["pivot_suggestions"] = (
                    pivot_suggestions.get("total_suggestions", 0)
                )
        ...
        if (
            include_charts
            and comprehensive_result.get("chart_analysis", {}).get("total_charts", 0)
            == 0
        ):
            overall_recommendations.append(
                "데이터 시각화 개선을 위해 차트 추가를 고려해보세요"
            )

        if (
            include_pivots
            and comprehensive_result.get("pivot_analysis", {}).get("total_pivots", 0)
            == 0
        ):
            overall_recommendations.append(
        ...
        if auto_generate_suggestions:
            total_suggestions = comprehensive_result.get("chart_suggestions", {}).get(
                "total_suggestions", 0
            ) + comprehensive_result.get("pivot_suggestions", {}).get(
                "total_suggestions", 0
            )
            if total_suggestions > 0:
                overall_recommendations.append(
                    f"총 {total_suggestions}개의 자동 생성 제안이 있습니다"
                )

        comprehensive_result["overall_recommendations"] = overall_recommendations

        return {"status": "success", "comprehensive_analysis": comprehensive_result}
