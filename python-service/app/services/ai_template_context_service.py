...
from ..models.template_metadata import EnhancedTemplateMetadata, FieldDefinition, WorkflowStep
...
class AITemplateContextService:
    ...
    async def generate_ai_context(self, metadata: EnhancedTemplateMetadata) -> Dict[str, Any]:
        """
        템플릿 메타데이터를 기반으로 AI가 사용할 컨텍스트 정보 생성
        """
        context = {
            "template_understanding": await self._generate_template_understanding(metadata),
    ...
    async def _generate_template_understanding(self, metadata: EnhancedTemplateMetadata) -> Dict[str, str]:
        """템플릿 이해를 위한 컨텍스트 생성"""
    ...
    async def _generate_field_guidance(self, metadata: EnhancedTemplateMetadata) -> Dict[str, Dict[str, Any]]:
        """각 필드에 대한 AI 가이던스 생성"""
        field_guidance = {}
        ...
        for section in metadata.sections:
            for field in section.fields:
                guidance = {
                    ...
                    "ai_prompt": await self._generate_field_ai_prompt(field, section, metadata)
                }
    ...
    async def _generate_validation_prompts(self, metadata: EnhancedTemplateMetadata) -> Dict[str, str]:
        """데이터 검증을 위한 프롬프트 생성"""
    ...
    async def _generate_user_assistance_prompts(self, metadata: EnhancedTemplateMetadata) -> Dict[str, str]:
        """사용자 지원을 위한 프롬프트 생성"""
    ...
    async def _generate_data_insight_prompts(self, metadata: EnhancedTemplateMetadata) -> Dict[str, str]:
    ...
    async def _generate_error_recovery_prompts(self, metadata: EnhancedTemplateMetadata) -> Dict[str, str]:
        """오류 복구를 위한 프롬프트 생성"""
    ...
    async def get_contextual_ai_prompt(
        ...
        # 작업 유형별 특화 프롬프트
        task_prompts = {
            "validation": metadata.ai_prompts.get("data_validation", "데이터를 검증해주세요."),
            "analysis": metadata.ai_prompts.get("trend_analysis", "데이터를 분석해주세요."), 
            "optimization": metadata.ai_prompts.get("optimization", "개선방안을 제시해주세요."),
