...
from app.services.fixing.strategies.na_fix_strategy import NAFixStrategy
from app.services.fixing.strategies.name_fix_strategy import NameFixStrategy
from app.services.fixing.strategies.ref_fix_strategy import RefFixStrategy
from app.services.fixing.strategies.value_fix_strategy import ValueFixStrategy
from app.services.fixing.strategies.circular_ref_fix_strategy import (
    CircularRefFixStrategy,
)
...
class IntegratedErrorFixer(IErrorFixer):
    ...
    def __init__(self, progress_reporter: Optional[IProgressReporter] = None):
        ...
        self.fix_strategies: List[IErrorFixStrategy] = [
            DivZeroFixStrategy(),
            NAFixStrategy(),  # #N/A 오류 수정
            NameFixStrategy(),  # #NAME? 오류 수정
            RefFixStrategy(),  # #REF! 오류 수정
            ValueFixStrategy(),  # #VALUE! 오류 수정
            CircularRefFixStrategy(),  # 순환 참조 오류 수정
        ]
    ...
    async def fix(self, error: ExcelError) -> FixResult:
        ...
        try:
            ...
            # 수정 적용 (context 매개변수 추가)
            context = {
                "sheet_info": {"max_row": 1000, "max_col": 26},  # 기본값
                "available_sheets": [],  # 실제 구현 시 워크북에서 추출
                "deleted_ranges": [],
            }
            result = await strategy.apply_fix(error, context)
    ...
    async def fix_batch(
        self, errors: List[ExcelError], strategy: str = "safe"
    ) -> Dict[str, Any]:
        """여러 오류 일괄 수정"""
        ...
        # 완료 보고
        if self.progress_reporter:
            await self.progress_reporter.complete_task("일괄 오류 수정", results)
    ...
    async def apply_fix_to_workbook(self, workbook: Any, fix_result: FixResult) -> bool:
        ...
        try:
            ...
            success = await self.workbook_loader.set_cell_value(
                workbook, sheet_name, cell_address, fix_result.fixed_formula
            )
    ...
    def remove_strategy(self, strategy_type: type):
        """수정 전략 제거"""
        self.fix_strategies = [
            s for s in self.fix_strategies if not isinstance(s, strategy_type)
        ]
    ...
    def _sort_errors_for_fixing(
        self, errors: List[ExcelError], strategy: str
    ) -> List[ExcelError]:
        """수정을 위한 오류 정렬"""
        if strategy == "safe":
            # 안전 모드: 신뢰도가 높은 것부터
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                processed_results.append(
                    self._create_error_result(errors[i], str(result))
                )
            else:
