...
class TestTemplateSelectionService:
    ...
    def test_get_template_by_id(self):
        ...
        # 존재하는 템플릿 조회
        result = template_selection_service.get_template_by_id(
    ...
    async def test_recommend_templates_text_only(self):
        ...
        result = await template_selection_service.recommend_templates(
            user_intent=financial_intent, user_tier="pro", max_recommendations=3
        )
        ...
        assert (
            "financial" in top_recommendation["category"]
            or "financial" in top_recommendation["template_id"]
        )
        assert top_recommendation["match_score"] > 0

        print(
            f"✅ 텍스트 기반 추천 테스트 통과: {len(result['recommendations'])}개 추천"
        )
    ...
    def test_tier_permission_check(self):
        ...
        complex_template = {"complexity_score": 9, "category": "academic_research"}

        # Basic 티어는 접근 불가
        assert not template_selection_service._check_tier_permission(
            complex_template, "basic"
        )

        # Pro 티어는 접근 가능
        assert template_selection_service._check_tier_permission(
            complex_template, "pro"
        )

        # Enterprise 티어는 접근 가능
        assert template_selection_service._check_tier_permission(
            complex_template, "enterprise"
        )
...
class TestTemplateExcelGenerator:
    ...
    async def test_generate_quarterly_financial_report(self):
        ...
        result = await template_excel_generator.generate_from_template(
            template_id="quarterly_financial_report", user_data=user_data
        )
    ...
    async def test_generate_sales_dashboard(self):
        ...
        result = await template_excel_generator.generate_from_template(
            template_id="sales_performance_dashboard", user_data=user_data
        )
    ...
    def test_sample_data_generation(self):
        ...
        assert all(
            key in financial_data[0] for key in ["category", "current", "previous"]
        )
...
class TestTemplateIntegration:
    ...
    async def test_end_to_end_template_workflow(self):
        ...
        recommendations = await template_selection_service.recommend_templates(
            user_intent=user_intent, user_tier="pro", max_recommendations=1
        )
    ...
    def test_template_metadata_integrity(self):
        ...
        for template_id, template in templates.items():
            ...
            assert (
                template_category in categories
            ), f"Template {template_id} has invalid category: {template_category}"
    ...
    def test_template_matching_algorithm_performance(self):
        ...
        for intent in test_intents:
            ...
            category_scores = template_selection_service._classify_intent_category(
                intent
            )
