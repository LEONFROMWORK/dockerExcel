...
class TestExcelChartAnalyzer:
    ...
    def sample_excel_file(self):
        ...
        data = {
            ...
            '날짜': [datetime(2024, i%12+1, 1) + timedelta(days=j*30) for i in range(30) for j in range(1)][:30]
        }
    ...
    def test_suggest_optimal_charts(self, sample_excel_file):
        ...
        try:
            ...
            # 시트별 제안 확인
            sheet_suggestions = suggestions['suggestions_by_sheet'].get('Sales_Data', {})
    ...
    def test_auto_generate_charts(self, sample_excel_file):
        ...
        try:
            result = excel_chart_analyzer.auto_generate_charts(sample_excel_file, max_charts_per_sheet=2)
            ...
            print(f"✅ 자동 차트 생성 테스트 성공: {result['total_charts_generated']}개 생성")
...
class TestExcelPivotAnalyzer:
    ...
    def test_suggest_optimal_pivots(self, sample_excel_file):
        ...
        try:
            ...
            required_fields = ['type', 'title', 'row_fields', 'value_fields', 'aggfunc', 'confidence', 'reason']
            for field in required_fields:
                assert field in first_suggestion
    ...
    def test_create_pivot_table(self, sample_excel_file):
        ...
        try:
            ...
            result = excel_pivot_analyzer.create_pivot_table(sample_excel_file, pivot_config)
    ...
    def test_create_cross_tabulation(self, sample_excel_file):
        ...
        try:
            ...
            result = excel_pivot_analyzer.create_cross_tabulation(sample_excel_file, crosstab_config)
    ...
    def test_auto_generate_pivots(self, sample_excel_file):
        ...
        try:
            result = excel_pivot_analyzer.auto_generate_pivots(sample_excel_file, max_pivots_per_sheet=2)
            ...
            print(f"✅ 자동 피벗테이블 생성 테스트 성공: {result['total_pivots_generated']}개 생성")
            
            # 생성된 파일 정리
            if 'final_output_file' in result and os.path.exists(result['final_output_file']):
...
class TestIntegratedFeatures:
    ...
    def comprehensive_excel_file(self):
        ...
        sales_data = {
            ...
            '제품': np.random.choice(['노트북', '태블릿', '스마트폰', '스마트워치', '이어폰'], 365),
        ...
        with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as tmp_file:
            with pd.ExcelWriter(tmp_file.name, engine='openpyxl') as writer:
                ...
                # 월별 요약 시트 추가
                monthly_summary = df_sales.groupby('월').agg({
                    '매출': 'sum',
                    '판매량': 'sum',
                    '고객만족도': 'mean'
                }).reset_index()
                ...
                # 제품별 요약 시트 추가
                product_summary = df_sales.groupby('제품').agg({
                    '매출': 'sum',
                    '판매량': 'sum',
                    '고객만족도': 'mean'
                }).reset_index()
    ...
    def test_comprehensive_analysis(self, comprehensive_excel_file):
        ...
        try:
            # 차트 제안 분석
            chart_suggestions = excel_chart_analyzer.suggest_optimal_charts(comprehensive_excel_file)
            assert chart_suggestions['total_suggestions'] > 0
            
            # 피벗테이블 제안 분석
            pivot_suggestions = excel_pivot_analyzer.suggest_optimal_pivots(comprehensive_excel_file)
            ...
            for sheet in expected_sheets:
                if sheet in chart_suggestions['suggestions_by_sheet']:
                    sheet_chart_suggestions = chart_suggestions['suggestions_by_sheet'][sheet]
                    assert 'suggested_charts' in sheet_chart_suggestions
                
                if sheet in pivot_suggestions['suggestions_by_sheet']:
                    sheet_pivot_suggestions = pivot_suggestions['suggestions_by_sheet'][sheet]
                    assert 'suggested_pivots' in sheet_pivot_suggestions
            
            print(f"✅ 종합 분석 테스트 성공")
