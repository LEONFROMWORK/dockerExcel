...
class AlgorithmOptimizer:
    ...
    def analyze_code(
        self, source_code: str, function_name: str = None
    ) -> List[CodeIssue]:
        """코드 분석 및 이슈 탐지"""
        issues = []
    ...
    def _analyze_complexity(
        self, tree: ast.AST, function_name: str = None
    ) -> List[CodeIssue]:
        """복잡도 분석"""
        issues = []
    ...
    def _analyze_large_data_structures(
        self, tree: ast.AST, function_name: str = None
    ) -> List[CodeIssue]:
        """대용량 데이터 구조 분석"""
        issues = []
    ...
    def _analyze_potential_memory_leaks(
        self, tree: ast.AST, function_name: str = None
    ) -> List[CodeIssue]:
        """잠재적 메모리 누수 분석"""
        issues = []
    ...
    def _generate_fix_suggestion(
        self, code_line: str, pattern_type: str, pattern_info: Dict[str, Any]
    ) -> str:
        """수정 제안 생성"""
    ...
    def optimize_function(
        self, func: Callable, test_data: Any = None
    ) -> OptimizationResult:
        """함수 최적화 및 성능 측정"""
        # 함수 소스 코드 추출
    ...
    def _measure_performance(
        ...
        try:
            ...
            logger.warning(
                f"Function execution failed during performance measurement: {e}"
            )
    ...
    def _calculate_estimated_improvement(self, issues: List[CodeIssue]) -> float:
        """예상 성능 개선도 계산"""
        improvement_factors = {"critical": 0.3, "high": 0.2, "medium": 0.1, "low": 0.05}
    ...
    def _generate_complexity_improvements(
    ...
    def generate_optimization_report(
        self, issues: List[CodeIssue], output_path: Path
    ) -> None:
        """최적화 보고서 생성"""
        try:
            # 심각도별 그룹화
