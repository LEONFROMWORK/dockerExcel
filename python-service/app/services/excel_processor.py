...
class ExcelProcessor:
    ...
    def restore_images_from_data(self, wb: Workbook, sheets_with_images: Dict[str, Any]) -> Dict[str, int]:
        """
            ...
            try:
                ...
                for img_data in images:
                    try:
                        ...
                        errors.append(f"Failed to restore image in {sheet_name}: {str(img_error)}")
                        continue
                
                restored_images[sheet_name] = restored_count
                
            except Exception as sheet_error:
                errors.append(f"Error processing sheet {sheet_name}: {str(sheet_error)}")
                continue
                    ...
                    try:
                        ...
                        if hasattr(img, 'ref') and img.ref:
                            try:
                                ...
                                image_info['data'] = base64.b64encode(img.ref).decode('utf-8')
                            except Exception as data_error:
                                logger.warning(f"Could not extract image data: {data_error}")
                                continue
