...
from .builders import TableBuilder, ChartBuilder, StyleBuilder
...
class ExcelBuilder:
    ...
    def create_table(
        ...
        return self.table_builder.create_table_legacy(
            worksheet, start_cell, end_cell, table_name, style
        )
    ...
    def add_data_validation(
        ...
        self.table_builder.add_data_validation_legacy(
            worksheet, cell_range, validation_type, formula1, formula2, allow_blank
        )
    ...
    def create_chart(
        ...
        self.chart_builder.create_chart_legacy(
            worksheet, chart_type, data_range, categories, title, position
        )
    ...
    def write_dataframe_with_formatting(
        self,
        worksheet: Worksheet,
        df: pd.DataFrame,
        start_row: int = 1,
        ...
        data_formats: Optional[Dict[str, str]] = None,
    ) -> Tuple[int, int]:
    ...
    def auto_adjust_column_width(
        worksheet: Worksheet, min_width: float = 8.0, max_width: float = 50.0
    ) -> None:
        """Static method for backward compatibility"""
        builder = ExcelBuilder()
