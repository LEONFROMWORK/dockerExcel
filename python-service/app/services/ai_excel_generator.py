...
class AIExcelGenerator:
    ...
    async def generate_from_natural_language(
        ...
        try:
            ...
            intent_analysis = await self._analyze_user_intent(
                user_request, context, language
            )

            # 2. Excel 구조 설계
            excel_structure = await self._design_excel_structure(
                intent_analysis, language
            )

            # 3. 데이터 스키마 생성
            data_schema = await self._generate_data_schema(
                excel_structure, intent_analysis
            )

            # 4. Excel 파일 생성
            excel_file = await self._create_excel_file(
                data_schema, excel_structure, intent_analysis
            )
    ...
    async def _analyze_user_intent(
        self, user_request: str, context: Optional[Dict[str, Any]], language: str
    ) -> Dict[str, Any]:
        """사용자 의도 분석"""
    ...
    async def _design_excel_structure(
        self, intent_analysis: Dict[str, Any], language: str
    ) -> Dict[str, Any]:
        """Excel 구조 설계"""
    ...
    async def _generate_data_schema(
        self, excel_structure: Dict[str, Any], intent_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        """데이터 스키마 생성"""
    ...
    async def _create_excel_file(
        ...
        for sheet_name, sheet_data in data_schema["sheets"].items():
            ...
            for col_idx, header in enumerate(headers, 1):
                ...
                ws.column_dimensions[col_letter].width = sheet_data["columns"][
                    col_idx - 1
                ].get("width", 15)
            ...
            if sample_data:
                table_range = (
                    f"A1:{get_column_letter(len(headers))}{len(sample_data) + 1}"
                )
    ...
    async def _enhance_with_ai(
        self, excel_file: str, intent_analysis: Dict[str, Any]
    ) -> str:
        """AI 기반 Excel 파일 향상"""
    ...
    async def _get_ai_suggestions(
        self, analysis: Dict[str, Any], intent_analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """AI 기반 개선 제안 생성"""
    ...
    async def _generate_sample_data(
        self, columns: List[Dict[str, Any]], intent_analysis: Dict[str, Any]
    ) -> List[List[Any]]:
        """샘플 데이터 생성"""

        intent_analysis.get("domain", "general")
    ...
    async def _create_chart(
        self, ws, chart_spec: Dict[str, Any], data: List[List[Any]]
    ):
        """차트 생성"""
            ...
            "sheets": [
                {
                    ...
                    "formatting": {"header_style": "modern"},
                }
            ]
        }
