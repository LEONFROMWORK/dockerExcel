...
from ..structure.excel_schema import ExcelStructure, SheetSchema, ColumnDefinition, DataType
...
class TemplateBridge:
    ...
    async def find_relevant_template(self, request: str, context: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    async def enhance_template_with_ai(
        ...
        # Optimize structure
        enhanced_structure = self._optimize_structure(
            enhanced_structure, analysis
        )
    ...
    def apply_template_patterns(
        self,
        generated_structure: ExcelStructure,
    ...
    async def _load_template_structure(self, template_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _convert_template_to_schema(self, template_structure: Dict[str, Any]) -> ExcelStructure:
        ...
        for sheet in structure.sheets:
            ...
            if "comparison" in requirements:
                sheet.columns.extend([
                    ColumnDefinition(name="전기대비", data_type=DataType.PERCENTAGE),
                    ColumnDefinition(name="목표대비", data_type=DataType.PERCENTAGE)
                ])
        ...
        if complexity == "simple":
            ...
            for sheet in structure.sheets:
                ...
                sheet.columns = [
                    ...
                    if col.data_type in essential_types
                ][:10]  # Limit columns
        ...
        for col_idx in range(1, worksheet.max_column + 1):
            ...
            if header_cell.value:
                col_info = {
                    ...
                    "width": worksheet.column_dimensions[openpyxl.utils.get_column_letter(col_idx)].width,
    ...
    def _get_column_samples(self, worksheet, col_idx: int, sample_size: int = 5) -> List[Any]:
    ...
    def _extract_structure_patterns(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "sheet_organization": analysis.get("sheets", {}).get("organization", "standard"),
    ...
    def _add_forecast_elements(self, structure: ExcelStructure) -> ExcelStructure:
        ...
        for sheet in structure.sheets:
            if any(col.data_type in [DataType.NUMBER, DataType.CURRENCY] for col in sheet.columns):
                sheet.columns.extend([
                    ColumnDefinition(name="예측_다음달", data_type=DataType.NUMBER),
                    ColumnDefinition(name="예측_3개월", data_type=DataType.NUMBER),
                    ColumnDefinition(name="예측_신뢰도", data_type=DataType.PERCENTAGE)
                ])
