...
class DataQualityEnhancer:
    ...
    def analyze_data_quality(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 데이터 프로파일 생성
            analysis_result["data_profile"] = self._generate_data_profile(
                analysis_result["sheets_analyzed"]
            )
            ...
            return {"error": f"데이터 품질 분석 실패: {str(e)}", "file_path": file_path}

    def enhance_data_quality(
        self, file_path: str, enhancement_options: Dict[str, bool] = None
    ) -> Dict[str, Any]:
        """데이터 품질 향상 적용"""
        ...
        try:
            ...
            enhancement_result["summary"]["quality_score_before"] = pre_analysis.get(
                "overall_quality_score", 0
            )
            ...
            for sheet_name in workbook.sheetnames:
                ...
                sheet_enhancements = self._enhance_sheet_quality(
                    sheet, sheet_name, enhancement_options
                )
                enhancement_result["enhancements_applied"].append(sheet_enhancements)
                enhancement_result["summary"]["sheets_processed"] += 1
                enhancement_result["summary"][
                    "total_changes"
                ] += sheet_enhancements.get("changes_count", 0)
            ...
            return {"error": f"데이터 품질 향상 실패: {str(e)}", "file_path": file_path}
    ...
    def _analyze_sheet_quality(self, sheet, sheet_name: str) -> Dict[str, Any]:
        ...
        sheet_analysis["data_stats"]["data_types_detected"] = self._analyze_data_types(
            df
        )
    ...
    def _enhance_sheet_quality(
        self, sheet, sheet_name: str, options: Dict[str, bool]
    ) -> Dict[str, Any]:
        """개별 시트의 데이터 품질 향상"""
        ...
        if options.get("remove_duplicates", True):
            ...
            if duplicate_count > 0:
                enhancement_result["enhancements"].append(
                    f"{duplicate_count}개 중복 행 제거"
                )
        ...
        for col_idx, col in enumerate(df.columns):
            ...
            if missing_count > 0:
                severity = (
                    ...
                    else "medium" if missing_percentage > 20 else "low"
                )
                ...
                issues.append(
                    DataQualityIssue(
                        ...
                        suggested_value=self._suggest_missing_data_treatment(
                            df[col], missing_percentage
                        ),
    ...
    def _detect_number_format_issues(
        self, df: pd.DataFrame, sheet_name: str
    ) -> List[DataQualityIssue]:
        """숫자 형식 문제 감지"""
        ...
        for col_idx, col in enumerate(df.columns):
            ...
            # 천 단위 구분자 불일치 검사
            comma_format = sum(
                1 for v in col_data if "," in v and re.match(r"[\d,]+", v)
            )
    ...
    def _detect_text_quality_issues(
        self, df: pd.DataFrame, sheet_name: str
    ) -> List[DataQualityIssue]:
        """텍스트 품질 문제 감지"""
    ...
    def _detect_phone_issues(
        ...
        for col_idx, col in enumerate(df.columns):
            ...
            if any(
                ...
                for keyword in ["phone", "tel", "전화", "휴대폰", "mobile"]
            ):
                col_data = df[col].dropna().astype(str)
    ...
    def _suggest_missing_data_treatment(
        self, series: pd.Series, missing_percentage: float
    ) -> str:
        """누락 데이터 처리 방법 제안"""
    ...
    def _generate_data_profile(self, sheets_analysis: List[Dict]) -> Dict[str, Any]:
        ...
        total_rows = sum(
            sheet.get("data_stats", {}).get("total_rows", 0)
            for sheet in sheets_analysis
        )
        total_columns = sum(
            sheet.get("data_stats", {}).get("total_columns", 0)
            for sheet in sheets_analysis
        )
        ...
        return {
            ...
            "average_quality_score": (
                sum(sheet.get("quality_score", 0) for sheet in sheets_analysis)
                / len(sheets_analysis)
                if sheets_analysis
                else 0
            ),
    ...
    def _generate_quality_recommendations(
        self, issues: List[DataQualityIssue], overall_score: float
    ) -> List[str]:
        """품질 개선 권장사항 생성"""
        ...
        if overall_score < 50:
            recommendations.append(
                "🚨 데이터 품질이 매우 낮습니다. 전면적인 데이터 정리가 필요합니다."
            )
        elif overall_score < 70:
            recommendations.append("⚠️ 데이터 품질 개선이 필요합니다.")
        elif overall_score < 90:
            recommendations.append(
                "✅ 데이터 품질이 양호하지만 몇 가지 개선사항이 있습니다."
            )
        else:
    ...
    def _analyze_data_types(self, df: pd.DataFrame) -> Dict[str, int]:
        ...
        type_counts = {"text": 0, "number": 0, "date": 0, "mixed": 0}
