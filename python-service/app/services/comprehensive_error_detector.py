...
class ComprehensiveErrorDetector:
    ...
    async def analyze_excel_file(self, file_path: str, options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if vba_result.get("has_vba"):
                for error in vba_result.get("errors", []):
                    errors.append(ErrorReport(
                        category=ErrorCategory.SYNTAX if error["category"] == "syntax" else ErrorCategory.LOGIC,
    ...
    def _check_formula_consistency(self, sheet) -> List[ErrorReport]:
        ...
        for col_idx in range(1, sheet.max_column + 1):
            col_formulas = []
            for row_idx in range(2, min(sheet.max_row + 1, 100)):  # Sample first 100 rows
            ...
            if len(col_formulas) > 5:  # Need enough formulas to detect pattern
                ...
                for row_idx, formula in col_formulas[1:]:
                    ...
                    if not self._formulas_similar(base_formula, formula, row_idx - col_formulas[0][0]):
                ...
                if inconsistent:
                    ...
                    errors.append(ErrorReport(
                        ...
                        location={
                            "sheet": sheet.title,
                            "cells": [f"{col_letter}{row}" for row in inconsistent[:5]]
                        },
        ...
        for row_idx in range(2, min(sheet.max_row + 1, 100)):
            for col_idx in range(1, sheet.max_column + 1):
                ...
                if isinstance(cell.value, (int, float)) and not cell.formula:
                    ...
                    if surrounding_formulas >= 4:  # Most surrounding cells have formulas
                        errors.append(ErrorReport(
                            category=ErrorCategory.PATTERN,
                            severity="low",
                            location={
    ...
    def _generate_fix_recommendations(self, errors: List[ErrorReport]) -> List[Dict[str, Any]]:
        ...
        for (category, severity), group_errors in sorted_groups[:5]:
            if len(group_errors) > 0:
                ...
                recommendations.append({
                    ...
                    "suggested_approach": sample_error.suggested_fix or "Manual review required",
                    "estimated_impact": self._estimate_impact(category, severity, len(group_errors))
                })
