...
async def get_template_details(template_id: str, i18n: I18nContext = Depends(get_i18n_context)):
    """특정 템플릿 상세 정보 조회"""
    ...
    try:
        ...
        if result["status"] != "success":
            error_message = i18n.get_error_message("template_not_found", template_id=template_id)
...
async def generate_excel_from_template(
    ...
    try:
        ...
        if result["status"] != "success":
            raise HTTPException(status_code=500, detail=result.get("error", "템플릿 생성 실패"))
        ...
        raise HTTPException(status_code=500, detail=f"Excel 생성 중 오류가 발생했습니다: {str(e)}")
...
async def auto_generate_excel_from_intent(
    ...
    try:
        ...
        if not recommendations.get("recommendations"):
            raise HTTPException(status_code=404, detail="적합한 템플릿을 찾을 수 없습니다")
        
        # 2. 최적 템플릿 선택 (자동 선택 또는 최상위)
        best_template = recommendations["recommendations"][0]
        template_id = best_template["template_id"]
        
        logger.info(f"선택된 템플릿: {template_id} (점수: {best_template['match_score']})")
        ...
        if generation_result["status"] != "success":
            raise HTTPException(status_code=500, detail=generation_result.get("error", "Excel 생성 실패"))
        ...
        raise HTTPException(status_code=500, detail=f"자동 생성 중 오류가 발생했습니다: {str(e)}")
...
async def submit_template_feedback(
    ...
    try:
        ...
        # 현재는 로그로만 기록
        logger.info(f"템플릿 피드백 수집: {json.dumps(feedback_data, ensure_ascii=False)}")
...
async def search_templates(
    ...
    try:
        ...
        for template_id, template in templates.items():
            # 티어 권한 확인
            if not template_selection_service._check_tier_permission(template, user_tier):
                continue
...
async def template_service_health():
    ...
    try:
        ...
        templates_count = len(template_selection_service.templates_metadata.get("templates", {}))
        ...
        return {
            ...
            "service_info": {
                ...
                "vectorizer_initialized": template_selection_service.template_vectors is not None
            }
        }
