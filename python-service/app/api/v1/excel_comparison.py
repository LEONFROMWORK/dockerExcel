...
async def compare_excel_files(
    ...
    try:
        ...
        # 비교 수행
        logger.info(f"파일 비교 시작: {expected_file.filename} vs {actual_file.filename}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"파일 비교 중 오류가 발생했습니다: {str(e)}"
        )
    finally:
        # 임시 파일 정리
            ...
            "fix_details": [
                {
                    ...
                    "message": "값이 성공적으로 수정되었습니다" if i % 5 != 0 else "수식 참조 오류로 수정 실패"
                }
...
async def generate_comparison_report(
    ...
    if output_format == "excel":
        # Excel 보고서 생성 (시뮬레이션)
        report_filename = f"comparison_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
