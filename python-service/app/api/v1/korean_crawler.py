...
async def start_crawling(
    request: CrawlingStartRequest, background_tasks: BackgroundTasks
):
    """
    한국어 템플릿 크롤링 시작
    """
    try:
        ...
        if (
            current_progress["current_status"] not in ["준비 중", "완료", "오류"]
...
async def stop_crawling():
    ...
    try:
        ...
        return {"status": "stopping", "message": "크롤링 중단 요청이 처리되었습니다"}
    ...
    try:
        ...
        for template in templates:
            ...
            categories[major]["subcategories"][minor]["templates"].append(
                {"title": template.title, "file_name": template.file_name}
            )
...
async def get_category_templates(category_major: str, category_minor: str):
    ...
    try:
        ...
        return {
            "status": "success",
            "category": {"major": category_major, "minor": category_minor},
            "templates": [
                {
                    "title": t.title,
...
async def get_crawling_statistics():
    ...
    try:
        ...
        if not templates:
            return {"status": "no_data", "message": "크롤링된 데이터가 없습니다"}
        ...
        basic_stats = {
            ...
            "success_rate": (
                round(
                    (progress.get("downloaded_templates", 0) / len(templates)) * 100, 2
                )
                if templates
                else 0
            ),
        ...
        if progress.get("start_time") and progress.get("duration"):
            time_stats = {
                ...
                "templates_per_minute": (
                    round(len(templates) / (progress.get("duration_minutes", 1)), 2)
                    if templates
                    else 0
                ),
            }
        ...
        return {
            ...
            "quality_metrics": {
                "templates_with_description": len(
                    [
                        t
                        for t in templates
                        if t.description and t.description != "설명 없음"
                    ]
                ),
                "templates_with_related_files": len(
                    [t for t in templates if t.related_files]
                ),
            },
        }
    ...
    try:
        ...
        try:
            ...
            return {
                ...
                "cookie_names": [
                    cookie.split("=")[0] for cookie in cookies[:5]
                ],  # 처음 5개만
            }
