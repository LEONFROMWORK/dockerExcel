...
def setup_logging(
    ...
    debug_mode = (
        ...
        else os.getenv("DEBUG_MODE", "False").lower() == "true"
    )
...
def get_performance_logger(name: str) -> logging.Logger:
    ...
    if perf_log_file and not any(
        isinstance(h, logging.FileHandler)
        and h.baseFilename.endswith("performance.log")
...
def create_debug_logger(name: str) -> logging.Logger:
    ...
    if os.getenv("DEBUG_MODE", "False").lower() == "true":
        ...
        if debug_file and not any(
            isinstance(h, logging.FileHandler) and h.baseFilename.endswith("debug.log")
            for h in logger.handlers
        ):
            # 디버그 로그 디렉토리 생성
