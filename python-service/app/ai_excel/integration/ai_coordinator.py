...
class AICoordinator:
    ...
    async def generate_excel(
        self, user_request: str, options: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            response = {
                ...
                "metadata": {
                    ...
                    "context_confidence": analysis.get("ai_insights", {}).get(
                        "confidence", 0
                    ),
                    "template_used": strategy.get("template_id"),
                    "sheets_created": len(structure.sheets),
                    "total_cells": sum(
                        sheet.row_count * len(sheet.columns)
    ...
    async def _build_context(
        self, user_request: str, options: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def _generate_structure(
        ...
        if not validation_results["valid"]:
            logger.warning(
                f"Structure validation warnings: {validation_results['warnings']}"
            )
            # Fix critical errors
    ...
    async def _create_excel(
        ...
        file_path = await generator.generate(
            request=context["original_text"], context=context
        )
    ...
    async def _quality_check(self, excel_file: str, structure: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            quality_report["checks"].append(
                {"name": "file_size", "passed": file_size > 0, "value": file_size}
            )
            ...
            if structure_validation["suggestions"]:
                quality_report["suggestions"].extend(
            ...
            quality_report["passed"] = False
            quality_report["checks"].append(
                {"name": "quality_check", "passed": False, "error": str(e)}
            )

        return quality_report

    def _get_sheet_constraints(
        self, sheet: Any, analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _fix_structure_errors(
        self, structure: Any, validation_results: Dict[str, Any]
    ) -> Any:
    ...
    def _store_generation_stats(
        self, generation_id: str, response: Dict[str, Any]
    ) -> None:
    ...
    def get_generation_stats(self) -> Dict[str, Any]:
        ...
        successful = sum(
            1 for stat in self.generation_stats.values() if stat["status"] == "success"
        )
        ...
        if successful > 0:
            durations = [
                stat["duration"]
                for stat in self.generation_stats.values()
        ...
        return {
            ...
            "success_rate": (
                successful / total_generations if total_generations > 0 else 0
            ),
