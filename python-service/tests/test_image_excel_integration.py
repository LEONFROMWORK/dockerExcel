...
class TestImageExcelIntegration:
    ...
    def test_convert_single_image_success(self, mock_service, sample_image):
        ...
        mock_instance.process_image_with_error_detection = AsyncMock(return_value={
            "status": "success",
            "image_analysis": {
                "type": "table",
                "confidence": 0.95,
    ...
    def test_batch_convert_success(self, mock_service, sample_image):
        ...
        # Mock 설정
        mock_instance = mock_service.return_value
        mock_instance.batch_process_images = AsyncMock(return_value={
            "status": "completed",
            "total_images": 2,
            "successful": 2,
            "failed": 0,
            ...
            "individual_results": [
                {"status": "success", "error_detection": {"summary": {"total_errors": 0}}},
                {"status": "success", "error_detection": {"summary": {"total_errors": 1}}}
            ],
            "failures": []
        })
    ...
    def test_analyze_image_type(self, mock_service, sample_image):
        ...
        files = {"file": ("test.png", sample_image, "image/png")}
        response = client.post(
            "/api/v1/image-excel-integration/analyze-image-type",
            files=files
        )
