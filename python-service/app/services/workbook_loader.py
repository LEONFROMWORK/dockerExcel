...
class OpenpyxlWorkbookLoader(IWorkbookLoader):
    ...
    async def set_cell_value(self, workbook: Any, sheet: str, cell: str, value: Any) -> bool:
        """셀 값을 설정합니다"""
        try:
            worksheet = workbook[sheet] if sheet in workbook.sheetnames else None
            if not worksheet:
    ...
    def get_formula(self, workbook: Any, sheet: str, cell: str) -> Optional[str]:
        ...
        try:
            ...
            # 수식이 있는 경우 반환
            if hasattr(cell_obj, 'value') and isinstance(cell_obj.value, str) and cell_obj.value.startswith('='):
                return cell_obj.value
