...
async def analyze_document_multimodal(
    ...
    try:
        ...
        if analysis_depth not in ["basic", "comprehensive", "detailed"]:
            raise HTTPException(
                status_code=400,
                detail="analysis_depth는 basic, comprehensive, detailed 중 하나여야 합니다",
            )
        ...
        result = await multimodal_ai_service.analyze_document_multimodal(
            image_path=temp_file_path, language=language, analysis_depth=analysis_depth
        )
        ...
        logger.info(
            f"멀티모달 분석 완료: {result.document_type} (신뢰도: {result.confidence:.2f})"
        )
...
async def classify_document_type(
    ...
    try:
        ...
        # 기본 분석으로 분류 정보 획득
        result = await multimodal_ai_service.analyze_document_multimodal(
            image_path=temp_file_path, language=language, analysis_depth="basic"
        )
        ...
        high_confidence_types = {
            doc_type: score
            for doc_type, score in classification.get("confidence_scores", {}).items()
            if score >= confidence_threshold
        }

        response = {
            "success": True,
            "file_info": {"filename": file.filename, "language": language},
            "classification": {
                ...
                "processing_suggestions": classification.get(
...
async def batch_multimodal_analysis(
    ...
    if len(files) > max_files:
        raise HTTPException(
            status_code=400, detail=f"최대 {max_files}개 파일까지 처리 가능합니다"
        )
    ...
    logger.info(
        f"배치 멀티모달 분석 완료: {results['summary']['successful']}/{len(files)} 성공"
    )
...
async def extract_visual_features(
    ...
    try:
        ...
        # 시각적 특징 추출
        visual_features = await multimodal_ai_service._extract_visual_features(
            temp_file_path
        )
