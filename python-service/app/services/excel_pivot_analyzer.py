...
class ExcelPivotAnalyzer:
            ...
            for sheet_name in workbook.sheetnames:
                ...
                if hasattr(sheet, '_pivots'):
                    for pivot in sheet._pivots:
                        pivot_info = {
                            'pivot_name': getattr(pivot, 'name', f'Pivot_{len(sheet_pivots) + 1}'),
                            ...
                            'column_fields': self._extract_pivot_fields(pivot, 'column'),
                            'data_fields': self._extract_pivot_fields(pivot, 'data'),
                            'filter_fields': self._extract_pivot_fields(pivot, 'filter')
                        }
    ...
    def suggest_optimal_pivots(self, file_path: str, sheet_name: str = None) -> Dict[str, Any]:
        """데이터에 최적화된 피벗테이블 제안"""
        ...
        try:
            ...
            for sheet_name, df in sheets_to_analyze:
                sheet_suggestions = self._analyze_data_for_pivot_suggestions(df, sheet_name)
                suggestions['suggestions_by_sheet'][sheet_name] = sheet_suggestions
                suggestions['total_suggestions'] += len(sheet_suggestions.get('suggested_pivots', []))
    ...
    def create_pivot_table(self, file_path: str, pivot_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 기존 워크북 로드
            with pd.ExcelWriter(file_path.replace('.xlsx', '_with_pivot.xlsx'), 
            ...
            return {
                'status': 'error',
                'message': f'피벗테이블 생성 실패: {str(e)}'
            }
    
    def auto_generate_pivots(self, file_path: str, max_pivots_per_sheet: int = 2) -> Dict[str, Any]:
        ...
        try:
            ...
            for sheet_name, sheet_suggestions in suggestions['suggestions_by_sheet'].items():
                suggested_pivots = sheet_suggestions.get('suggested_pivots', [])[:max_pivots_per_sheet]
    ...
    def create_cross_tabulation(self, file_path: str, crosstab_config: Dict[str, Any]) -> Dict[str, Any]:
        """교차표 생성"""
    ...
    def _generate_pivot_recommendations(self, pivot_details: List[Dict]) -> List[str]:
        ...
        if len(pivot_details) == 0:
            recommendations.append("데이터 요약 분석을 위해 피벗테이블 생성을 고려해보세요")
        
        # 복잡도가 낮은 피벗테이블 확인
        simple_pivots = [
            pivot for pivot in pivot_details 
            if len(pivot.get('row_fields', [])) + len(pivot.get('column_fields', [])) <= 2
        ]
        
        if len(simple_pivots) > len(pivot_details) * 0.7:
            recommendations.append("더 복잡한 다차원 분석을 위해 추가 필드를 활용할 수 있습니다")
        
        # 데이터 필드가 없는 피벗테이블
        pivots_without_data = [
            pivot for pivot in pivot_details 
            if not pivot.get('data_fields')
        ]
        
        if pivots_without_data:
            recommendations.append(f"{len(pivots_without_data)}개의 피벗테이블에 집계 데이터 필드 추가를 고려해보세요")
        
        return recommendations
    
    def _analyze_data_for_pivot_suggestions(self, df: pd.DataFrame, sheet_name: str) -> Dict[str, Any]:
        ...
        # 3. 시계열 기반 피벗
        if len(datetime_columns) >= 1 and len(categorical_columns) >= 1 and len(numeric_columns) >= 1:
            # 날짜를 년/월/분기로 그룹화
            suggestions.append({
                'type': 'time_series_pivot',
                'title': f'{sheet_name} - 시간별 {categorical_columns[0]} 분석',
                'row_fields': [categorical_columns[0]],
                'column_fields': [datetime_columns[0]],  # 실제로는 년/월로 변환 필요
                'value_fields': [numeric_columns[0]],
                'aggfunc': 'sum',
                'confidence': 0.7,
