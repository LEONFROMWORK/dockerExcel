...
async def detect_charts(
    ...
    try:
        ...
        # 신뢰도 필터링
        filtered_charts = [
            chart for chart in charts if chart.confidence >= confidence_threshold
        ]

        logger.info(
            f"감지된 차트: {len(charts)}개, 필터링 후: {len(filtered_charts)}개"
        )
...
async def detect_charts_with_visualization(
    ...
    try:
        ...
        # 시각화 생성
        visualized_image = chart_detector.visualize_chart_detection(
            image, filtered_charts
        )
        ...
        if return_format == "image":
            ...
            return Response(
                ...
                headers={
                    "Content-Disposition": "inline; filename=chart_detection_result.png"
                },
            )
        else:
            ...
            result = {
                ...
                "visualization": {"format": "base64_png", "data": image_base64},
            }
...
async def analyze_chart_data_points(
    ...
    try:
        ...
        if not charts:
            return {"success": False, "message": "감지된 차트가 없습니다", "charts": []}
