...
class ExcelChartAnalyzer:
    ...
    def suggest_optimal_charts(
        ...
        try:
            ...
            for sheet_name, df in sheets_to_analyze:
                sheet_suggestions = self._analyze_data_for_chart_suggestions(
                    df, sheet_name
                )
                suggestions["suggestions_by_sheet"][sheet_name] = sheet_suggestions
                suggestions["total_suggestions"] += len(
                    sheet_suggestions.get("suggested_charts", [])
                )
    ...
    def create_chart(
        self, file_path: str, chart_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """지정된 설정으로 차트 생성"""
        ...
        try:
            ...
            return {"status": "error", "message": f"차트 생성 실패: {str(e)}"}

    def auto_generate_charts(
        self, file_path: str, max_charts_per_sheet: int = 3
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for sheet_name, sheet_suggestions in suggestions[
                ...
                suggested_charts = sheet_suggestions.get("suggested_charts", [])[
            ...
            return {"status": "error", "message": f"자동 차트 생성 실패: {str(e)}"}
    ...
    def _generate_chart_recommendations(self, chart_details: List[Dict]) -> List[str]:
        ...
        if len(set(chart_types)) == 1 and len(chart_types) > 1:
            recommendations.append(
                "다양한 차트 유형을 사용하여 데이터를 더 효과적으로 표현할 수 있습니다"
            )

        charts_without_title = [
            chart for chart in chart_details if not chart.get("title")
        ]
        if charts_without_title:
            recommendations.append(
                f"{len(charts_without_title)}개의 차트에 제목을 추가하면 가독성이 향상됩니다"
            )

        return recommendations

    def _analyze_data_for_chart_suggestions(
    ...
    def _create_single_chart(
        self, workbook: openpyxl.Workbook, chart_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """단일 차트 생성 (내부 메서드)"""
        ...
        try:
            ...
            if sheet_name not in workbook.sheetnames:
                return {
                    "status": "error",
                    "message": f"시트 '{sheet_name}'를 찾을 수 없습니다",
                }
