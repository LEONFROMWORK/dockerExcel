...
from ...structure.excel_schema import SheetSchema, ColumnDefinition, DataType, FormulaDefinition, FormulaType
...
class IncomeStatementGenerator(BaseFinanceGenerator):
    ...
    def _define_statement_structure(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"account": "영업이익", "level": 1, "formula": "매출총이익 - 판매관리비 계"},
            {"account": "영업외수익", "level": 2, "formula": None},
            {"account": "영업외비용", "level": 2, "formula": None},
            {"account": "법인세차감전순이익", "level": 1, "formula": "영업이익 + 영업외수익 - 영업외비용"},
            {"account": "법인세비용", "level": 2, "formula": None},
            {"account": "당기순이익", "level": 1, "formula": "법인세차감전순이익 - 법인세비용"}
        ]
    ...
    def _generate_industry_specific_amounts(self, industry: str, size: str) -> Dict[str, float]:
    ...
    def _apply_statement_formulas(self, data: pd.DataFrame, periods: List[str]) -> pd.DataFrame:
                ...
                for period in periods:
                    if account == "매출총이익":
                        data_dict[period][account] = (
                            data_dict[period]["매출액"] - 
                            data_dict[period]["매출원가"]
                        )
                    elif account == "판매관리비 계":
                        data_dict[period][account] = (
                            data_dict[period]["판매비"] + 
                            data_dict[period]["관리비"]
                        )
                    elif account == "영업이익":
                        data_dict[period][account] = (
    ...
    def _add_variance_analysis(self, data: pd.DataFrame, periods: List[str]) -> pd.DataFrame:
        ...
        if len(periods) >= 2:
            ...
            data["증감률(%)"] = data.apply(
                lambda row: (
                    ((row[current_period] - row[previous_period]) / abs(row[previous_period]) * 100)
                    if row[previous_period] != 0 else 0
                ),
                axis=1
            ).round(1)
