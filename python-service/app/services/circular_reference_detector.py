...
class CircularReferenceDetector:
    ...
    def _extract_cell_references(self, formula: str, current_sheet: str, workbook) -> Set[str]:
        ...
        function_names = {
            ...
            'INDEX', 'MATCH', 'SUMIF', 'COUNTIF', 'AND', 'OR', 'NOT', 'IFERROR',
            'CONCATENATE', 'LEFT', 'RIGHT', 'MID', 'LEN', 'TRIM', 'DATE', 'NOW'
        }
        ...
        for match in re.finditer(named_range_pattern, formula):
            ...
            if name.upper() not in function_names:
                ...
                if hasattr(workbook, 'defined_names') and name in workbook.defined_names:
    ...
    def _expand_range(self, range_ref: str, sheet_name: str) -> List[str]:
        ...
        try:
            if ':' in range_ref:
                ...
                # 너무 큰 범위는 제한
                if (column_index_from_string(end_col) - column_index_from_string(start_col) > 100 or
                    end_row - start_row > 100):
                    return []
                
                for row in range(start_row, end_row + 1):
                    for col_idx in range(column_index_from_string(start_col), 
    ...
    def _analyze_circular_chains(self, cycles: List[List[str]]) -> List[CircularReferenceChain]:
        ...
        for cycle in cycles:
            ...
            circular_chains.append(CircularReferenceChain(
                cells=cycle,
    ...
    def _generate_description(self, cycle: List[str], chain_type: str) -> str:
        ...
        if chain_type == "direct":
            ...
            return f"{len(cycle)}개의 셀이 순환 참조 체인을 형성하고 있습니다: " + " → ".join(cycle) + " → " + cycle[0]
