...
async def comprehensive_excel_analysis(
    ...
    confidence_level: str = Query(
        "balanced",
        description="Analysis confidence level: conservative, balanced, aggressive",
    ),
    focus_areas: Optional[List[str]] = Query(
        None, description="Specific areas to focus on"
    ),
    user_concern: Optional[str] = Query(
        None, description="User's specific concern about the file"
    ),
):
    """
    ...
    try:
        ...
        analysis_options = {
            ...
            and file.filename.lower().endswith((".xlsm", ".xlsb")),
        }
        ...
        logger.info(
            f"Comprehensive analysis completed for {file.filename}: {result['total_errors']} errors found"
        )
...
async def analyze_with_business_context(
    file: UploadFile = File(...),
    business_context: str = Query(
        ..., description="Business context: financial, inventory, sales, hr, general"
    ),
    custom_rules: Optional[str] = Query(
        None, description="Custom business rules in JSON format"
    ),
    language: str = Query("ko", description="Language for analysis: ko, en"),
    export_format: Optional[str] = Query(
        None, description="Export format: json, excel, pdf"
    ),
):
    """
    ...
    if custom_rules:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid JSON format for custom rules"
            )
    ...
    try:
        ...
        if custom_rules_dict:
            for rule_name, rule_content in custom_rules_dict.items():
                prompt_manager.add_custom_rule(
                    rule_name, rule_content, business_context
                )
...
async def quick_excel_validation(
    ...
    try:
        ...
        return {"success": True, "data": result, "quick_validation": True}
...
async def compare_analysis_results(
    ...
    comparison_type: str = Query(
        "errors", description="Type of comparison: errors, structure, all"
    ),
):
    """
    ...
    try:
        ...
        for file_path, filename in files:
            result = await detector.analyze_excel_file(
                file_path,
                {
                    "include_basic": True,
...
async def _generate_excel_report(
    analysis_result: Dict[str, Any], original_filename: str
) -> str:
    ...
    if analysis_result["errors"]:
        ...
        for row_idx, error in enumerate(analysis_result["errors"], 2):
            ...
            errors_sheet.cell(
                row=row_idx, column=4, value=str(error.get("location", {}))
            )
            errors_sheet.cell(row=row_idx, column=5, value=error.get("description", ""))
            errors_sheet.cell(
                row=row_idx, column=6, value=error.get("suggested_fix", "")
            )
...
def _compare_results(
    result1: Dict[str, Any], result2: Dict[str, Any], comparison_type: str
) -> Dict[str, Any]:
    ...
    comparison["summary"] = {
        ...
        "error_reduction_rate": (len(comparison["fixed_errors"]) / max(len(errors1), 1))
        * 100,
    }
