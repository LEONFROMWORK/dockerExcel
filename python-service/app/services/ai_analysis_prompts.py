...
class AIAnalysisPrompts:
    ...
    def generate_analysis_prompt(
        ...
        if analysis_options and "focus_areas" in analysis_options:
            prompt_parts.append("\n=== 중점 분석 영역 ===")
            prompt_parts.append(
                self._format_focus_areas(analysis_options["focus_areas"])
            )
    ...
    def add_custom_rule(
        self, rule_name: str, rule_content: str, context: Optional[str] = None
    ):
    ...
    def _get_applicable_thresholds(
        self, context_type: AnalysisContext
    ) -> Dict[str, Any]:
    ...
    def _format_file_context(self, file_context: Dict[str, Any]) -> str:
        ...
        for sheet in file_context.get("sheets", [])[:3]:  # First 3 sheets
            ...
            context_info.append(
                f"  - 크기: {sheet.get('rows', 0)} x {sheet.get('columns', 0)}"
            )
            context_info.append(
                f"  - 데이터 타입: {', '.join(sheet.get('data_types', []))}"
            )
    ...
    def generate_domain_specific_prompt(
        self, domain: str, file_data: Dict[str, Any], user_concern: Optional[str] = None
    ) -> str:
        """Generate domain-specific analysis prompt"""

        base_prompt = self.generate_analysis_prompt(
            file_data, {"focus_areas": [domain]}
        )
    ...
    def adjust_for_confidence(
        self, prompt: str, confidence_level: str = "balanced"
    ) -> str:
        ...
        return prompt + confidence_adjustments.get(
            confidence_level, confidence_adjustments["balanced"]
        )
