...
class VBAGenerator:
    ...
    def _load_template_metadata(self) -> Dict[str, Any]:
        ...
        metadata_path = (
            Path(__file__).parent.parent / "templates" / "vba" / "metadata.json"
        )
        try:
    ...
    def _init_validation_patterns(self):
        ...
        self.security_patterns = {
            ...
            "file_system": re.compile(
                r'CreateObject\s*\(\s*["\']Scripting\.FileSystemObject', re.IGNORECASE
            ),
            "wscript": re.compile(
                r'CreateObject\s*\(\s*["\']WScript\.Shell', re.IGNORECASE
            ),
            "registry": re.compile(r"RegRead|RegWrite|RegDelete", re.IGNORECASE),
            "external_data": re.compile(
                r"ADODB\.|XMLHttp|InternetExplorer\.Application", re.IGNORECASE
            ),
        }

        # Performance issue patterns
        self.performance_patterns = {
            "select_activate": re.compile(
                r"\.Select\s*\n.*?\.Activate|\.Activate\s*\n.*?\.Select", re.IGNORECASE
            ),
            "redundant_select": re.compile(
                r"\.Select\s*\n\s*Selection\.", re.IGNORECASE
            ),
            "nested_loops_cells": re.compile(
                r"For.*?\n.*?For.*?\n.*?Cells\(", re.IGNORECASE | re.DOTALL
            ),
            "copy_paste": re.compile(
        ...
        self.syntax_patterns = {
            "unclosed_blocks": re.compile(
                r"(If|For|While|With|Sub|Function)(?!.*?End\s*\1)", re.IGNORECASE
            ),
            "undeclared_variables": re.compile(
                r"(?<!Dim\s)(?<!Set\s)(\w+)\s*=(?!=)", re.IGNORECASE
            ),
            "missing_error_handling": re.compile(
                r"Sub\s+\w+.*?End\s+Sub(?!.*?On\s+Error)", re.IGNORECASE | re.DOTALL
            ),
        }
    ...
    def validate_vba_code(
        self, vba_code: str, check_security: bool = True, check_performance: bool = True
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_error_handling": bool(
                re.search(r"On\s+Error", vba_code, re.IGNORECASE)
            ),
        }
    ...
    def fix_common_issues(self, vba_code: str) -> str:
        ...
        fixed_code = re.sub(
            r"(\w+)\.Select\s*\n\s*Selection\.", r"\1.", fixed_code, flags=re.IGNORECASE
        )
    ...
    def analyze_vba_code(self, vba_code: str) -> Dict[str, Any]:
        ...
        analysis = {"issues": [], "suggestions": [], "metrics": {}}

        # Basic metrics
        lines = vba_code.splitlines()
        analysis["metrics"]["total_lines"] = len(lines)
        analysis["metrics"]["code_lines"] = len(
        ...
        if re.search(
            r"\.Select.*?\.Activate|\.Activate.*?\.Select",
            vba_code,
            re.IGNORECASE | re.DOTALL,
        ):
            analysis["issues"].append("Using Select/Activate pattern")
            analysis["suggestions"].append(
                "Replace Select/Activate with direct object references"
            )
    ...
    def get_improvement_suggestions(self, vba_code: str) -> List[Dict[str, str]]:
        ...
        if re.search(
            r"\.Copy.*?\.Paste(?!Special)", vba_code, re.IGNORECASE | re.DOTALL
        ):
            suggestions.append(
                {
