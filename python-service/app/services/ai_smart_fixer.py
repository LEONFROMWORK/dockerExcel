...
class AISmartFixer:
    ...
    async def smart_fix_file(
        ...
        try:
            ...
            context_analysis = await self._analyze_file_context(
                workbook, file_analysis, user_intent
            )
            results["ai_analysis"]["context"] = context_analysis

            # 2. 지능형 데이터 타입 추론 및 수정
            logger.info("지능형 데이터 타입 추론...")
            type_fixes = await self._smart_data_type_inference(
                workbook, context_analysis
            )
            ...
            formula_optimizations = await self._smart_formula_optimization(
                workbook, context_analysis
            )
            results["ai_fixes_applied"] += formula_optimizations["count"]
            results["fixes_applied"].extend(formula_optimizations["details"])
            results["confidence_scores"]["formulas"] = formula_optimizations[
                "confidence"
            ]

            # 4. 컨텍스트 기반 데이터 정리
            logger.info("컨텍스트 기반 데이터 정리...")
            data_improvements = await self._contextual_data_cleanup(
                workbook, context_analysis
            )
            ...
            suggestions = await self._generate_optimization_suggestions(
                workbook, context_analysis, file_analysis
            )
            results["optimization_suggestions"] = suggestions

            logger.info(
                f"AI 스마트 수정 완료: {results['ai_fixes_applied']}개 수정, {results['context_improvements']}개 개선"
            )
    ...
    async def _analyze_file_context(
        self,
        workbook: openpyxl.Workbook,
        file_analysis: Dict[str, Any],
        user_intent: str = None,
        ...
        try:
            response = await self.openai_service.chat_completion(
                messages=[{"role": "user", "content": context_prompt}], temperature=0.3
            )
    ...
    def _extract_sheet_sample(
        self, sheet: openpyxl.worksheet.worksheet.Worksheet, sample_size: int = 5
    ) -> List[List]:
        """시트에서 샘플 데이터 추출"""
    ...
    def _identify_headers(
        ...
        if sheet.max_row > 0:
            for col in range(1, min(sheet.max_column + 1, 20)):  # 최대 20개 컬럼
                ...
                headers.append(
                    str(cell_value) if cell_value is not None else f"Column_{col}"
                )
    ...
    def _analyze_column_types(
        ...
        for col in range(1, min(sheet.max_column + 1, 20)):
            ...
            if not sample_values:
                ...
                isinstance(v, str) and self._is_date_like(v) for v in sample_values
            ):
    ...
    async def _smart_data_type_inference(
        self, workbook: openpyxl.Workbook, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """지능형 데이터 타입 추론 및 변환"""

        fixes = {"count": 0, "details": [], "confidence": 0.0}
        ...
        for sheet in workbook.worksheets:
            try:
                ...
                # 추천 사항에 따라 데이터 타입 변환
                for col_letter, rec in recommendations.get(
                    "column_recommendations", {}
                ).items():
    ...
    def _convert_column_type(
        ...
        for row in range(2, sheet.max_row + 1):  # 헤더 제외
            ...
            try:
                ...
                logger.warning(
                    f"셀 {get_column_letter(col_index)}{row} 타입 변환 실패: {str(e)}"
                )

        return converted

    async def _smart_formula_optimization(
        self, workbook: openpyxl.Workbook, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """스마트 수식 최적화"""
            ...
            for opt in optimizations.get("optimizations", []):
                if opt["confidence"] > 0.8:  # 높은 신뢰도만 적용
                    ...
                    for formula_info in formula_cells:
                        if (
                            f"{formula_info['sheet']}!{formula_info['address']}"
                            == location
                        ):
                            try:
                                ...
                                logger.warning(
                                    f"수식 최적화 적용 실패 {location}: {str(e)}"
                                )
                            break

            # 평균 신뢰도 계산
            confidences = [
                opt["confidence"] for opt in optimizations.get("optimizations", [])
            ]
            fixes["confidence"] = (
                sum(confidences) / len(confidences) if confidences else 0.0
            )
    ...
    async def _contextual_data_cleanup(
        self, workbook: openpyxl.Workbook, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """컨텍스트 기반 데이터 정리"""

        improvements = {"count": 0, "details": []}
    ...
    def _apply_sales_cleanup(
        ...
        for col_idx, header in enumerate(headers, 1):
            if any(
                ...
                for keyword in ["quantity", "qty", "수량", "amount", "금액"]
            ):
    ...
    def _apply_inventory_cleanup(
        ...
        for col_idx, header in enumerate(headers, 1):
            if any(
                keyword in header.lower() for keyword in ["sku", "code", "코드", "id"]
            ):
                for row in range(2, sheet.max_row + 1):
                    cell = sheet.cell(row=row, column=col_idx)
                    if cell.value and isinstance(cell.value, str):
    ...
    async def _generate_data_insights(
        ...
        try:
            ...
            response = await self.openai_service.chat_completion(
                messages=[{"role": "user", "content": insights_prompt}], temperature=0.4
            )
