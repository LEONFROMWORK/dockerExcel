...
class PatternApplier(ABC):
    ...
    @abstractmethod
    def apply(self, data: pd.DataFrame, col_name: str, pattern_spec: Dict[str, Any]) -> pd.DataFrame:
...
class SeasonalPatternApplier(PatternApplier):
    ...
    def __init__(self):
        self.seasonal_func = lambda base, period: base * (1 + 0.1 * np.sin(2 * np.pi * period / 12))
...
class PatternApplierFactory:
    ...
    def apply_patterns(self, data: pd.DataFrame, patterns: Dict[str, Dict[str, Any]]) -> pd.DataFrame:
