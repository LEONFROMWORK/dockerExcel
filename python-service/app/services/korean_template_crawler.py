...
from ..models.template_metadata import TemplateCategory
...
class KoreanTemplateCrawler:
        ...
        try:
                ...
                if "=" in cookie_line and not cookie_line.strip().startswith("#"):
                    for cookie_pair in cookie_line.split(";"):
                        ...
                        if "=" in cookie_pair:
                            ...
                            cookies_applied += 1
                            logger.info(
                                f"쿠키 설정: {name.strip()}={value.strip()[:10]}..."
                            )
            ...
            async with self.session.get(self.base_url) as response:
                ...
                if is_logged_in:
                    ...
                    logger.warning("⚠️  쿠키는 적용되었지만 로그인 상태 불확실")
                    # 일단 계속 진행해보기
                    return True
            ...
            async with self.session.get(self.login_url) as response:
                ...
                if login_form:
                    ...
                    if not action.startswith("http"):
                        action = (
                            f"https://www.yesform.com{action}"
                            if action.startswith("/")
                            else f"https://www.yesform.com/z_n/member/{action}"
                        )
                ...
                for hidden in hidden_inputs:
                    if hidden.get("name"):
                        ...
                        logger.info(
                            f"히든 필드 추가: {hidden.get('name')} = {hidden.get('value', '')}"
                        )
    ...
    async def _get_major_categories(self) -> List[Dict[str, str]]:
        ...
        logger.info(
            f"✅ 대분류 {len(categories)}개 수집: {[cat['name'] for cat in categories]}"
        )
        return categories
    ...
    async def _crawl_major_category(self, major_category: Dict[str, str]):
        ...
        try:
            ...
            async with self.session.get(major_category["url"]) as response:
                ...
                # 중분류 수집 및 처리
                minor_categories = await self._get_minor_categories(
                    soup, major_category["url"]
                )
                logger.info(
                    f"📁 '{major_category['name']}'에서 중분류 {len(minor_categories)}개 발견"
                )
                ...
                if not minor_categories:
                    ...
                    logger.info(
                        f"📋 '{major_category['name']}'에서 템플릿 {len(template_links)}개 발견"
                    )

                    for template_link in template_links:
                        await self._process_template(
                            major_category["name"], "일반", template_link
                        )
                else:
                    # 각 중분류별로 템플릿 수집
                    for minor_category in minor_categories:
                        await self._crawl_minor_category_with_pagination(
                            major_category["name"], minor_category
                        )

        except Exception as e:
            logger.error(f"❌ '{major_category['name']}' 크롤링 실패: {e}")

    async def _get_minor_categories(
            ...
            if value and next_label:
                ...
                if category_name and value != "all":  # 'all' 제외
                    minor_categories.append(
                        {"name": category_name, "value": value, "base_url": base_url}
                    )

        return minor_categories

    async def _crawl_minor_category(
        self, major_name: str, minor_category: Dict[str, str]
    ):
        """중분류별 템플릿 크롤링"""
        ...
        try:
            # 중분류 필터 적용된 URL 생성
            category_url = (
                f"{minor_category['base_url']}&format={minor_category['value']}"
            )
            ...
            async with self.session.get(category_url) as response:
                ...
                logger.info(
                    f"📋 '{minor_category['name']}'에서 템플릿 {len(template_links)}개 발견"
                )

                # 각 템플릿 상세 정보 수집 및 다운로드
                for template_link in template_links:
                    await self._process_template(
                        major_name, minor_category["name"], template_link
                    )
        ...
        try:
            ...
            for section in filter_sections:
                ...
                for label in section.find_all("label"):
                    ...
                    if (
                        input_elem
                        and input_elem.get("value")
                        and input_elem.get("value") != "all"
                    ):
    ...
    async def _crawl_minor_category_with_pagination(
        self, major_name: str, minor_category: Dict[str, str]
    ):
        """중분류별 페이지네이션 포함 크롤링"""
            ...
            try:
                ...
                async with self.session.get(page_url) as response:
                    ...
                    if not template_links:
                        logger.info(
                            f"📄 '{minor_category['name']}' 페이지 {page}: 더 이상 템플릿이 없음"
                        )
                        break

                    logger.info(
                        f"📄 '{minor_category['name']}' 페이지 {page}: {len(template_links)}개 템플릿 발견"
                    )
                ...
                logger.error(
                    f"❌ 중분류 '{minor_category['name']}' 페이지 {page} 처리 실패: {e}"
                )
                break
    ...
    async def _get_template_links(self, soup: BeautifulSoup) -> List[str]:
        ...
        if not list_section:
            # 대체 선택자들 시도
            list_section = soup.find("div", id="ListSection") or soup.find(
                "section", class_="list-section"
            )
            if not list_section:
    ...
    async def _process_template(
        self, major_category: str, minor_category: str, template_url: str
    ):
        """개별 템플릿 처리"""
        try:
            # 상세 페이지 접속
    ...
    async def _extract_template_info(
    ...
    async def _download_template(self, template_info: Dict[str, str]) -> Optional[Path]:
        ...
        try:
            ...
            category_dir = (
                self.download_dir
                / template_info["category_major"]
                / template_info["category_minor"]
            )
            ...
            async with self.session.get(
                ...
                logger.info(
                    f"다운로드 응답: {response.status}, Content-Type: {response.headers.get('Content-Type', 'unknown')}"
                )
                ...
                if await self._verify_excel_file(file_path):
                    ...
                    file_path.unlink()
                    logger.warning(
                        f"❌ 잘못된 파일 형식 (HTML/오류 페이지): {file_name}"
                    )
                    return None
    ...
    async def _save_crawling_results(self):
        ...
        try:
            ...
            results_file = (
                self.metadata_dir
                / f"crawling_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
...
"""
            ...
            if self.failed_downloads:
                ...
                for failed in self.failed_downloads:
                    ...
                    report_content += (
                        f"  - URL: {failed.get('source_url', 'Unknown')}\n"
                    )

            # 보고서 파일 저장
            report_file = (
                self.reports_dir
                / f"crawling_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
            )
