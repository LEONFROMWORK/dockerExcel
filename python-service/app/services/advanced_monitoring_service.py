...
from app.services.strategic_batch_manager import get_batch_manager
...
class ConsoleAlertHandler(AlertHandler):
    ...
    async def send_alert(self, alert: MonitoringAlert) -> bool:
        ...
        logger.warning(
            f"{icon} [{alert.severity.value.upper()}] {alert.title}: {alert.message}"
        )
        return True
...
class BatchJobMetricCollector(MetricCollector):
    ...
    def collect_metrics(self) -> List[PerformanceMetric]:
        ...
        try:
            ...
            # 성공률
            total_processed = (
                scheduler_stats["total_completed"] + scheduler_stats["total_failed"]
            )
            if total_processed > 0:
                success_rate = (
                    scheduler_stats["total_completed"] / total_processed
                ) * 100
                metrics.append(
                    PerformanceMetric(
...
class JobProgressTracker:
    ...
    def update_job_progress(
        ...
        with self.lock:
            ...
            # 에러/경고 메시지 추가
            if error_message:
                progress_info.error_messages.append(
                    f"{current_time.strftime('%H:%M:%S')}: {error_message}"
                )
                # 최근 5개만 유지
            ...
            if warning_message:
                progress_info.warnings.append(
                    f"{current_time.strftime('%H:%M:%S')}: {warning_message}"
                )
                # 최근 5개만 유지
            ...
            self.progress_history[job_id].append(
                (current_time.timestamp(), progress_percent)
            )
    ...
    def _calculate_estimated_time(
        self, job_id: str, progress_info: JobProgressInfo
    ) -> None:
        """예상 완료 시간 계산"""
        ...
        if len(recent_entries) >= 2:
            ...
            if time_diff > 0 and progress_diff > 0:
                ...
                # 예상 완료 시간 (초)
                if progress_rate > 0:
                    progress_info.estimated_remaining_seconds = int(
    ...
    def cleanup_completed_jobs(self) -> None:
        ...
        with self.lock:
            ...
            completed_jobs = [
                job_id
                for job_id in self.progress_info.keys()
...
class AdvancedMonitoringService:
    ...
    def start_monitoring(self) -> None:
        ...
        self.monitoring_thread = threading.Thread(
            target=self._monitoring_loop, name="AdvancedMonitoring", daemon=True
        )
    ...
    def _check_metric_alert(
        self, metric: PerformanceMetric, threshold: float, current_time: datetime
    ) -> None:
        """메트릭 기반 알림 확인"""
    ...
    def _get_system_summary(self, latest_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "batch.active.jobs" in latest_metrics:
                summary["active_jobs"] = int(
                    latest_metrics["batch.active.jobs"]["value"]
                )
            ...
            # 전체 건강 상태 판단
            if (
                summary["cpu_usage"] > 90
                or summary["memory_usage"] > 95
                or len(self.active_alerts) > 5
            ):
                summary["overall_health"] = "critical"
            elif (
                summary["cpu_usage"] > 80
                or summary["memory_usage"] > 85
                or len(self.active_alerts) > 2
            ):
