...
class TemplateExcelGenerator:
    ...
    async def generate_from_template(
        ...
        try:
            ...
            generator_method = getattr(
                self, f"_generate_{template_id}", self._generate_generic_template
            )
    ...
    def _create_revenue_trend_chart(self, ws, data, position):
        ...
        data_ref = Reference(
            ws, min_col=2, min_row=start_row, max_row=start_row + len(periods) - 1
        )
        categories = Reference(
            ws, min_col=1, min_row=start_row, max_row=start_row + len(periods) - 1
        )
    ...
    def _create_profitability_chart(self, ws, data, position):
        ...
        data_ref = Reference(
            ws, min_col=2, min_row=start_row, max_row=start_row + len(rates) - 1
        )
        categories = Reference(
            ws, min_col=1, min_row=start_row, max_row=start_row + len(rates) - 1
        )
    ...
    def _create_cost_composition_chart(self, ws, data, position):
        ...
        data_ref = Reference(
            ws, min_col=2, min_row=start_row, max_row=start_row + len(costs) - 1
        )
        categories = Reference(
            ws, min_col=1, min_row=start_row, max_row=start_row + len(costs) - 1
        )
    ...
    def _create_sales_dashboard_sheet(self, ws, user_data: Dict[str, Any] = None):
        ...
        # 영업사원 데이터
        sales_data = (
            user_data.get("sales_team_data", [])
            if user_data
            else self._generate_sample_sales_data()
        )
        ...
        for i, person in enumerate(sales_data, 10):
            ...
            # 달성률 계산
            achievement = (
                person["actual"] / person["target"] if person["target"] > 0 else 0
            )
    ...
    def _create_monthly_sales_chart(self, ws, user_data: Dict[str, Any] = None):
        ...
        monthly_sales = (
            user_data.get("monthly_sales", [50000, 60000, 55000, 70000, 80000, 75000])
            if user_data
            else [50000, 60000, 55000, 70000, 80000, 75000]
        )
        ...
        for i, (month, sales) in enumerate(zip(months, monthly_sales)):
            ...
            ws.cell(
                row=chart_start_row + 1 + i, column=chart_start_col + 1, value=sales
            )
        ...
        data_ref = Reference(
            ...
            max_row=chart_start_row + len(months),
        )
        categories = Reference(
            ws,
        ...
        # 차트 위치 설정
        ws.add_chart(
            chart, f"{get_column_letter(chart_start_col + 3)}{chart_start_row}"
        )
    ...
    def _create_sales_detail_sheet(self, ws, user_data: Dict[str, Any] = None):
        ...
        detail_data = (
            user_data.get("sales_detail", [])
            if user_data
            else self._generate_sample_sales_detail()
        )
        ...
        for i, record in enumerate(detail_data, 3):
            ws.cell(
                row=i,
                column=1,
                value=record.get("date", datetime.now().strftime("%Y-%m-%d")),
            )
    ...
    async def _generate_generic_template(
        ...
        try:
            ...
            output_path = self._save_workbook(
                wb, template.get("id", "generic_template")
            )
    ...
    def _populate_generic_data(self, ws, user_data: Dict[str, Any]):
        ...
        # 딕셔너리 형태의 데이터인 경우
            ...
            if data_list:
                # 첫 번째 레코드에서 키 추출
                headers = (
                    list(data_list[0].keys()) if isinstance(data_list[0], dict) else []
                )
        ...
        for row in range(6, 6 + min_rows):
            ...
            for col in range(2, len(headers) + 1):
                if "수치" in headers[col - 1]:
                    ...
                    ws.cell(
                        row=row, column=col, value=datetime.now().strftime("%Y-%m-%d")
                    )
                else:
