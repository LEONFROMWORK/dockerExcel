...
class BatchJobAnalyticsEngine(AnalyticsEngine):
    ...
    def _get_jobs_by_period(self, period: AnalyticsPeriod) -> List[Any]:
        ...
        period_jobs = [
            job for job in self.batch_manager.scheduler.completed_jobs
    ...
    def _calculate_financial_kpis(self, jobs: List[Any]) -> List[KPIMetric]:
        ...
        # ROI 계산
        roi = ((total_revenue_impact - total_cost) / total_cost * 100) if total_cost > 0 else 0
    ...
    def _calculate_operational_kpis(self, jobs: List[Any]) -> List[KPIMetric]:
        ...
        # 처리량 (시간당)
        if jobs:
            time_span = max(job.completed_at for job in jobs) - min(job.created_at for job in jobs)
    ...
    def _calculate_quality_kpis(self, jobs: List[Any]) -> List[KPIMetric]:
        ...
        # 재시도 없이 성공한 작업 비율
        first_try_success = [job for job in jobs if job.retry_count == 0 and job.status == JobStatus.COMPLETED]
        ...
        sla_compliance_rate = (len(sla_compliant_jobs) / len(jobs_with_sla)) * 100 if jobs_with_sla else 100
    ...
    def _calculate_job_type_efficiency(self, jobs: List[Any]) -> Dict[str, float]:
        ...
        for job_type, type_jobs in jobs_by_type.items():
            ...
            # 효율성 점수 계산 (0-100)
            success_rate = len([j for j in type_jobs if j.status == JobStatus.COMPLETED]) / len(type_jobs)
            ...
            for job in type_jobs:
                if job.started_at and job.completed_at:
                    ...
                    if expected_time > 0:
                        efficiency = min(expected_time / actual_time, 2.0)  # 최대 2배 효율
                        time_efficiency += efficiency
                        valid_time_comparisons += 1
            
            avg_time_efficiency = time_efficiency / valid_time_comparisons if valid_time_comparisons > 0 else 1.0
    ...
    def _generate_underperformance_insight(self, kpi: KPIMetric) -> Optional[BusinessInsight]:
        """저성과 KPI 인사이트 생성"""
    ...
    def _generate_overperformance_insight(self, kpi: KPIMetric) -> Optional[BusinessInsight]:
        """고성과 KPI 인사이트 생성"""
    ...
    def _analyze_job_type_patterns(self, jobs: List[Any]) -> Dict[str, Dict[str, Any]]:
        ...
        for job_type, type_jobs in jobs_by_type.items():
            success_count = len([j for j in type_jobs if j.status == JobStatus.COMPLETED])
            success_rate = (success_count / len(type_jobs)) * 100 if type_jobs else 0
            
            avg_revenue = statistics.mean([j.business_metrics.revenue_impact for j in type_jobs]) if type_jobs else 0
            avg_cost = statistics.mean([j.business_metrics.processing_cost for j in type_jobs]) if type_jobs else 0
            
            # 성과 점수 계산 (0-100)
            performance_score = (success_rate * 0.4 + 
                               min(avg_revenue / max(avg_cost, 1) * 10, 60)) # ROI 기반 점수
    ...
    def _generate_trend_insights(self, data: Dict[str, Any]) -> List[BusinessInsight]:
        ...
        # KPI 트렌드 분석
        kpis = data.get('kpis', [])
        declining_kpis = [kpi for kpi in kpis if kpi.change_rate and kpi.change_rate < -10]
...
class ExecutiveReportGenerator(ReportGenerator):
    ...
    def _extract_key_metrics(self, kpis: List[KPIMetric]) -> Dict[str, Any]:
        ...
        priority_kpis = ['roi_percentage', 'success_rate', 'throughput_per_hour', 'avg_cost_per_job']
    ...
    def _generate_performance_summary(self, kpis: List[KPIMetric]) -> Dict[str, Any]:
        ...
        for category, category_kpis in categories.items():
            avg_achievement = statistics.mean([
                kpi.achievement_rate for kpi in category_kpis 
                if kpi.achievement_rate is not None
            ]) if category_kpis else 0
    ...
    def _recommend_next_steps(self, insights: List[BusinessInsight], kpis: List[KPIMetric]) -> List[str]:
        """다음 단계 권장"""
        next_steps = []
        
        # 긴급 조치가 필요한 KPI 확인
        critical_kpis = [kpi for kpi in kpis if kpi.target and kpi.achievement_rate and kpi.achievement_rate < 70]
...
class BusinessAnalyticsService:
    ...
    def generate_comprehensive_analysis(self, period: AnalyticsPeriod) -> Dict[str, Any]:
        """종합 비즈니스 분석 생성"""
        ...
        # 경영진 보고서 생성
        executive_summary = self.report_generator.generate_executive_summary(analysis_data)
        ...
        total_revenue_impact = sum(job.business_metrics.revenue_impact for job in period_jobs) if period_jobs else 0
        total_cost = sum(job.business_metrics.processing_cost for job in period_jobs) if period_jobs else 1
        average_roi = ((total_revenue_impact - total_cost) / total_cost * 100) if total_cost > 0 else 0
        efficiency_score = 85.0  # 기본 효율성 점수
        cost_savings = max(0, total_revenue_impact - total_cost)
        
        # KPI를 카테고리별로 분류
        kpi_metrics = {"financial": [], "operational": [], "quality": [], "efficiency": []}
        for kpi in kpis:
            ...
            if category in kpi_metrics:
                kpi_metrics[category].append({
                    ...
                    "achievement_rate": kpi.achievement_rate,
                    "change_rate": kpi.change_rate,
                    "trend": "increasing" if kpi.change_rate > 0 else "decreasing" if kpi.change_rate < 0 else "stable"
                })
        ...
        comprehensive_analysis = {
            ...
            "recommendations": self._generate_actionable_recommendations(insights, kpis)
        }
    ...
    def calculate_roi_metrics(self) -> Dict[str, Any]:
        """ROI 메트릭 계산"""
        period_jobs = self.analytics_engine._get_jobs_by_period(AnalyticsPeriod.LAST_24_HOURS)
        ...
        for job_type, type_jobs in jobs_by_type.items():
            ...
            type_roi = ((type_revenue - type_cost) / type_cost * 100) if type_cost > 0 else 0
    ...
    def _generate_roi_analysis(self, jobs: List[Any]) -> Dict[str, Any]:
        ...
        # 손익분기점 추정
        daily_cost = total_investment / max(len(set(job.created_at.date() for job in jobs)), 1)
        daily_return = total_return / max(len(set(job.created_at.date() for job in jobs)), 1)
        ...
        return {
            ...
            "investment_efficiency": total_return / total_investment if total_investment > 0 else 0
        }
    ...
    def _generate_operational_metrics(self, jobs: List[Any]) -> Dict[str, Any]:
        ...
        # 작업 유형별 분석
        job_type_stats = defaultdict(lambda: {"count": 0, "success": 0, "total_revenue": 0, "total_cost": 0})
        ...
        for job in jobs:
            ...
            job_type_stats[job_type]["total_revenue"] += job.business_metrics.revenue_impact
            job_type_stats[job_type]["total_cost"] += job.business_metrics.processing_cost
        
        # 통계 계산
        for stats in job_type_stats.values():
            stats["success_rate"] = (stats["success"] / stats["count"]) * 100 if stats["count"] > 0 else 0
            stats["roi"] = ((stats["total_revenue"] - stats["total_cost"]) / stats["total_cost"]) * 100 if stats["total_cost"] > 0 else 0
    ...
    def _generate_actionable_recommendations(self, insights: List[BusinessInsight], kpis: List[KPIMetric]) -> List[Dict[str, Any]]:
        """실행 가능한 권장사항 생성"""
        recommendations = []
        ...
        for insight in high_priority_insights:
            for action in insight.recommended_actions:
                recommendations.append({
                    ...
                    "timeline": "immediate" if insight.impact_level == "high" else "short_term"
                })
        
        # KPI 기반 권장사항
        underperforming_kpis = [kpi for kpi in kpis if kpi.target and kpi.achievement_rate and kpi.achievement_rate < 80]
    ...
    def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_analysis": max(self.analysis_cache.keys()) if self.analysis_cache else None
        }
