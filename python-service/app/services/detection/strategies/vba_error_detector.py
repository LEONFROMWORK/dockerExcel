...
class VBAErrorDetector(IErrorDetector):
    """VBA 코드 오류 감지기"""
    
    def __init__(self):
        # VBA 에러 패턴
        self.error_patterns = {
            'undeclared_variable': re.compile(r'(?<!Dim\s)(?<!Private\s)(?<!Public\s)(?<!Const\s)\b([a-zA-Z_]\w*)\s*=', re.IGNORECASE),
            'missing_error_handling': re.compile(r'^(?!.*On\s+Error).*\b(Open|Kill|MkDir|Name|FileCopy)\b', re.IGNORECASE | re.MULTILINE),
            'deprecated_function': re.compile(r'\b(DoEvents|SendKeys|Application\.Volatile)\b', re.IGNORECASE),
            'infinite_loop_risk': re.compile(r'Do\s+While\s+True|While\s+True|Do\s*\n(?!.*Loop\s+Until)', re.IGNORECASE | re.DOTALL),
            'missing_option_explicit': re.compile(r'^(?!Option\s+Explicit)', re.IGNORECASE),
            'hardcoded_path': re.compile(r'["\'](?:[A-Za-z]:[\\/]|\\\\)[^"\']+["\']'),
            'sql_injection_risk': re.compile(r'\.Execute\s*\(\s*["\'].*["\'].*&.*&.*["\']', re.IGNORECASE),
            'unhandled_object': re.compile(r'Set\s+(\w+)\s*=.*(?!.*Set\s+\1\s*=\s*Nothing)', re.IGNORECASE | re.DOTALL)
        }
        
        # VBA 구문 패턴
        self.syntax_patterns = {
            'sub_function': re.compile(r'(?:Sub|Function)\s+(\w+)\s*\(([^)]*)\)', re.IGNORECASE),
            'end_sub_function': re.compile(r'End\s+(?:Sub|Function)', re.IGNORECASE),
            'variable_declaration': re.compile(r'(?:Dim|Private|Public|Const)\s+(\w+)(?:\s+As\s+(\w+))?', re.IGNORECASE),
    ...
    async def detect(self, workbook: Any) -> List[ExcelError]:
        ...
        try:
            ...
            logger.error(f"VBA 분석 오류: {str(e)}")
    ...
    async def _analyze_vba_module(self, module_name: str, code: str) -> List[ExcelError]:
        """VBA 모듈 분석"""
        errors = []
        ...
        if not re.search(r'Option\s+Explicit', code, re.IGNORECASE):
            errors.append(self._create_error(
                module_name, 1, "VBA Best Practice Violation",
        ...
        for proc_name, proc_code in procedures.items():
            ...
            if self.error_patterns['missing_error_handling'].search(proc_code):
                if not re.search(r'On\s+Error', proc_code, re.IGNORECASE):
                    ...
                    errors.append(self._create_error(
                        module_name, line_num, "VBA Logic Error",
    ...
    def _check_variable_declarations(self, module_name: str, code: str) -> List[ExcelError]:
        """변수 선언 확인"""
        errors = []
        declared_vars = set()
        ...
        for i, line in enumerate(lines):
            ...
            for var in assignments:
                if var.lower() not in declared_vars and not self._is_builtin(var):
                    errors.append(self._create_error(
                        module_name, i + 1, "VBA Syntax Error",
                        f"선언되지 않은 변수 사용: {var}",
                        "high", True,
    ...
    def _check_hardcoded_paths(self, module_name: str, code: str) -> List[ExcelError]:
        ...
        for match in self.error_patterns['hardcoded_path'].finditer(code):
            ...
            path = match.group(0)
            errors.append(self._create_error(
                module_name, line_num, "VBA Best Practice Violation",
                f"하드코딩된 경로: {path}",
                "medium", True,
                "설정 파일이나 환경 변수 사용 권장"
            ))
    ...
    def _check_dangerous_apis(self, module_name: str, code: str) -> List[ExcelError]:
        ...
        for api, severity in self.dangerous_apis.items():
            ...
            for match in pattern.finditer(code):
                line_num = code[:match.start()].count('\n') + 1
                errors.append(self._create_error(
                    module_name, line_num, "VBA Security Risk",
                    f"위험한 API 사용: {api}",
                    severity, False,
    ...
    def _check_structural_issues(self, module_name: str, code: str) -> List[ExcelError]:
        ...
        if with_count != end_with_count:
            errors.append(self._create_error(
                module_name, 0, "VBA Syntax Error",
    ...
    def _is_builtin(self, var_name: str) -> bool:
        ...
        builtins = {
            ...
            'me', 'err', 'debug', 'application', 'activeworkbook',
            'activesheet', 'selection', 'range', 'cells'
        }
    ...
    def _create_error(self, module_name: str, line_num: int, error_type: str,
                     message: str, severity: str, auto_fixable: bool,
                     suggested_fix: str) -> ExcelError:
        """오류 객체 생성"""
        return ExcelError(
            id=f"{module_name}_line{line_num}_{error_type.replace(' ', '_').lower()}",
            type=error_type,
            sheet=module_name,  # VBA 모듈명을 시트 필드에 저장
            cell=f"Line {line_num}",
            formula=None,
            value=None,
            message=message,
            severity=severity,
