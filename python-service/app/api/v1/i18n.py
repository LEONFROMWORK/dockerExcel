...
async def get_translations(
    ...
    namespace: Optional[str] = Query(None, description="네임스페이스 (예: common, templates)"),
    i18n: I18nContext = Depends(get_i18n_context)
):
    """번역 데이터 조회"""
    
    try:
        target_language = language if language and i18n_service.validate_language(language) else i18n.language
...
async def get_localized_progress_stages(
    ...
    try:
        ...
        localized_stages = {}
        for stage in stages:
            localized_stages[stage] = i18n_service.get_progress_stage_text(stage, target_language)
