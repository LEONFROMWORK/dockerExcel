    ...
    def get_current_stats(cls) -> 'MemoryStats':
        ...
        try:
            ...
            return cls(
                ...
                peak_memory=getattr(process_memory_info, 'peak_wset', process_memory_info.rss)
            )
        ...
        with self.lock:
            return {
                **self.stats,
                ...
                'pool_utilization': len(self.in_use_objects) / max(self.stats['current_size'], 1),
                'config': {
...
class AdaptiveGCStrategy(GarbageCollectionStrategy):
    ...
    def collect(self) -> Dict[str, Any]:
        ...
        # 평균 수집 시간 업데이트
        total_time = self.collection_stats['avg_collection_time'] * (self.collection_stats['total_collections'] - 1)
        self.collection_stats['avg_collection_time'] = (total_time + collection_time) / self.collection_stats['total_collections']
        ...
        logger.info(f"가비지 컬렉션 완료: {memory_freed / 1024 / 1024:.2f}MB 해제, {collection_time:.3f}초")
        return result
    ...
    def get_comprehensive_stats(self) -> Dict[str, Any]:
        ...
        return {
            'current_memory': {
                ...
                'available_memory_gb': current_memory.available_memory / 1024 / 1024 / 1024,
        ...
        for name, pool in self.memory_pools.items():
            if hasattr(pool, '_cleanup_idle_objects'):
                ...
                pool_cleanup_stats[name] = {
                    ...
                    'objects_cleaned': before_stats['available_objects'] - after_stats['available_objects']
                }
