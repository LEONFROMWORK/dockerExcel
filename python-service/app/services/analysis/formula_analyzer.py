...
class FormulaAnalyzer:
            ...
            for cell in row:
                if (
                    ...
                    analysis = await self._analyze_formula(
                        cell.value, cell.coordinate, sheet_name, sheet
                    )
    ...
    async def _analyze_formula(
        self, formula: str, cell: str, sheet: str, worksheet: Any
    ) -> FormulaAnalysis:
        """개별 수식 분석"""
    ...
    def _calculate_complexity(
        self, formula: str, functions: List[str], cells: List[str], ranges: List[str]
    ...
    async def _calculate_dependency_depth(
        self, cell: str, sheet: str, worksheet: Any, referenced_cells: List[str]
    ) -> int:
        """의존성 깊이 계산"""
        ...
        for ref_cell in referenced_cells:
            try:
                ...
                if (
                    ref_cell_obj.value
                    and isinstance(ref_cell_obj.value, str)
                    and ref_cell_obj.value.startswith("=")
                ):
                    # 재귀적으로 깊이 계산 (간소화된 버전)
    ...
    def _evaluate_performance_impact(
        self, functions: List[str], ranges: List[str], is_volatile: bool, is_array: bool
    ) -> str:
        """성능 영향 평가"""
    ...
    def _aggregate_analyses(
        self, analyses: List[FormulaAnalysis], dependency_graph: nx.DiGraph
    ) -> WorkbookAnalysis:
        """분석 결과 집계"""
        ...
        most_used = sorted(function_count.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]

        # 휘발성 수식
        volatile_formulas = [f"{a.sheet}!{a.cell}" for a in analyses if a.is_volatile]

        # 배열 수식
        array_formulas = [f"{a.sheet}!{a.cell}" for a in analyses if a.is_array_formula]
    ...
    def _get_bottleneck_reason(self, analysis: FormulaAnalysis) -> str:
        ...
        high_impact = [
            f for f in analysis.functions_used if f in self.high_impact_functions
        ]
        if high_impact:
    ...
    def _identify_optimization_opportunities(
        self, analyses: List[FormulaAnalysis]
    ) -> List[Dict[str, Any]]:
        """최적화 기회 식별"""
        ...
        # VLOOKUP 사용 패턴
        vlookup_cells = [a for a in analyses if "VLOOKUP" in a.functions_used]
