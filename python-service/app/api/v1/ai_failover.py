...
@router.get("/status", summary="AI 페일오버 시스템 전체 상태")
async def get_system_status():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"시스템 상태 조회 중 오류가 발생했습니다: {str(e)}"
        )


@router.get("/models/available", summary="사용 가능한 AI 모델 목록")
...
@router.post("/health-check", summary="AI 모델 헬스체크 실행")
async def perform_health_check(
    background_tasks: BackgroundTasks, request: HealthCheckRequest
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"헬스체크 실행 중 오류가 발생했습니다: {str(e)}"
        )


@router.post("/test", summary="AI 모델 페일오버 테스트")
    ...
    try:
        ...
        if request.required_tier:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid tier: {request.required_tier}"
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"페일오버 테스트 중 오류가 발생했습니다: {str(e)}"
        )


@router.get("/providers", summary="AI 제공업체 목록 및 상태")
async def get_providers_status():
    ...
    try:
        ...
        for provider_name, provider_info in status["providers"].items():
            providers_detail[provider_name] = {**provider_info, "models": []}
...
@router.get("/metrics", summary="AI 페일오버 시스템 메트릭스")
