...
class FastFormulaFixer(SmartFormulaFixer):
    ...
    def _precompile_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "cell_ref": re.compile(
                r"(?:(?:\'[^\']+\'|[A-Za-z_]\w*)!)?(?:\$?[A-Z]+\$?\d+(?::\$?[A-Z]+\$?\d+)?|\$?[A-Z]+:\$?[A-Z]+|\d+:\d+)"
            ),
            "currency": re.compile(r"[₩$€£¥]"),
            "date": re.compile(r"\d{4}[-/]\d{2}[-/]\d{2}|\d{2}[-/]\d{2}[-/]\d{4}"),
            "array_formula": re.compile(r"^\{=.*\}$"),
            "dynamic_array": re.compile(
                r"SORT|FILTER|UNIQUE|SEQUENCE|RANDARRAY", re.IGNORECASE
            ),
    ...
    def _get_cached_fix(
        self, formula: str, error_type: str
    ...
    def _cache_fix_result(
        self, formula: str, error_type: str, result: FormulaFixResult
    ):
        """수정 결과를 캐시에 저장"""
        # 캐시 크기 제한
        ...
        hash_key = self._get_formula_hash(formula, error_type)
        self.fix_cache[hash_key] = FixCache(formula_hash=hash_key, fix_result=result)

    async def fix_formula_errors_batch(
    ...
    async def _process_single_error_async(
        self, workbook, error: ExcelError, fix_results: Dict[str, Any]
    ):
        """단일 오류를 비동기로 처리"""
        try:
            ...
            if cached_fix:
                ...
                # 결과 캐싱
                if fix_result:
                    self._cache_fix_result(
                        str(cell.value), error.error_type, fix_result
                    )
    ...
    async def _fix_single_formula_error_fast(
    ...
    async def _optimize_single_formula_fast(
        self, cell, error: ExcelError
    ) -> Optional[FormulaFixResult]:
        """최적화된 단일 수식 최적화"""
    ...
    def _formula_matches_pattern_fast(self, formula: str, pattern_name: str) -> bool:
        ...
        pattern_checks = {
            "vlookup_to_xlookup": lambda f: self.pattern_cache["vlookup"].search(f)
            is not None,
            ...
            "array_formula": lambda f: self.pattern_cache["array_formula"].match(f)
            is not None,
        }
