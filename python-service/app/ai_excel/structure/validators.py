...
from .excel_schema import ExcelStructure, SheetSchema
from .sheet_validators import (
    SheetNameValidator,
    ColumnValidator,
    FormulaValidator,
    ChartValidator,
    SheetSuggestionGenerator,
)
...
class SchemaValidator:
    ...
    def _merge_results(
        self, main_results: Dict[str, Any], new_results: Dict[str, Any]
    ) -> None:
        ...
        for rel in structure.relationships:
            ...
            if rel.source_sheet not in sheet_names:
                results["errors"].append(
                    f"Relationship references non-existent sheet: {rel.source_sheet}"
                )
                results["valid"] = False

            if rel.target_sheet not in sheet_names:
                results["errors"].append(
                    f"Relationship references non-existent sheet: {rel.target_sheet}"
                )
                results["valid"] = False
            ...
            if not self._is_valid_range(rel.source_range):
                results["errors"].append(
                    f"Invalid source range in relationship: {rel.source_range}"
                )
                results["valid"] = False

            if not self._is_valid_range(rel.target_range):
                results["errors"].append(
                    f"Invalid target range in relationship: {rel.target_range}"
                )
                results["valid"] = False
    ...
    def _cell_exists_in_sheet(self, cell_ref: str, sheet: SheetSchema) -> bool:
        ...
        # Check bounds
        return (
            1 <= col_num <= len(sheet.columns) and 1 <= row_num <= sheet.row_count + 10
        )  # Allow some buffer
