...
class Settings(BaseSettings):
    ...
    RAILS_INTERNAL_API_KEY: str = Field(default="development-internal-key")
    ...
    GCP_PROJECT_ID: str = Field(default="excel-unified")
    GCP_LOCATION: str = Field(default="us-central1")
    VERTEX_AI_EMBEDDING_MODEL: str = Field(default="text-embedding-004")

    # Embedding Service Configuration
    EMBEDDING_SERVICE: str = Field(default="vertex_ai")  # vertex_ai only
    ...
    ALLOWED_EXTENSIONS: List[str] = Field(default=[".xlsx", ".xls", ".csv"])
    ...
    VISION_TEMPERATURE: float = Field(
        default=0.1
    )  # Lower temperature for more accurate vision analysis
