...
class FormulaErrorDetector(IErrorDetector):
            ...
            for row in sheet.iter_rows():
                for cell in row:
                    if cell.data_type == 'f':  # Formula cell
                        cell_errors = await self._check_cell_formula(cell, sheet.title)
    ...
    async def _check_cell_formula(self, cell, sheet_name: str) -> List[ExcelError]:
        """셀의 수식 검사"""
        errors = []
    ...
    def _check_function_syntax(self, formula: str) -> Optional[str]:
        ...
        known_functions = {
            ...
            'HLOOKUP', 'INDEX', 'MATCH', 'SUMIF', 'COUNTIF', 'CONCATENATE',
    ...
    def _create_error_from_value(self, cell, sheet_name: str) -> Optional[ExcelError]:
        ...
        for error_type, pattern in self.error_patterns.items():
            if pattern.match(value):
                return self._create_error(cell, sheet_name, error_type, 
                                        self._get_error_message(error_type))
        
        return None
    
    def _create_error(self, cell, sheet_name: str, error_type: ExcelErrorType, 
                     message: str) -> ExcelError:
        """오류 객체 생성"""
        return ExcelError(
