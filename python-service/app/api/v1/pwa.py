...
async def get_manifest(request: Request, i18n: I18nContext = Depends(get_i18n_context)):
    """PWA 매니페스트 파일 제공"""

    try:
        manifest_path = os.path.join(
            os.path.dirname(__file__), "../../pwa/manifest.json"
        )
...
async def get_service_worker():
    ...
    try:
        sw_path = os.path.join(os.path.dirname(__file__), "../../pwa/service_worker.js")
        ...
        raise HTTPException(
            status_code=500, detail="Service Worker를 로드할 수 없습니다"
        )


@router.post("/install")
async def track_pwa_install(
    install_data: Dict[str, Any], i18n: I18nContext = Depends(get_i18n_context)
):
    """PWA 설치 추적"""

    try:
        install_data.get("user_agent", "unknown")
...
async def unsubscribe_push_notifications(
    subscription_id: str, i18n: I18nContext = Depends(get_i18n_context)
):
    """푸시 알림 구독 해제"""
...
async def send_push_notification(
    ...
    try:
        ...
        background_tasks.add_task(
            send_push_to_subscribers, message, notification_type, target_users
        )
...
async def clear_pwa_cache(
    cache_types: List[str] = None, i18n: I18nContext = Depends(get_i18n_context)
):
    """PWA 캐시 삭제"""
...
async def check_pwa_update(
    current_version: str = "1.0.0", i18n: I18nContext = Depends(get_i18n_context)
):
    """PWA 업데이트 확인"""
...
async def get_offline_capabilities(i18n: I18nContext = Depends(get_i18n_context)):
    ...
    try:
        offline_features = {
            "cached_pages": ["/", "/upload", "/templates", "/dashboard", "/settings"],
            "cached_apis": [
                "/api/v1/health",
...
async def send_push_to_subscribers(
    message: str, notification_type: str, target_users: List[str]
):
    """구독자들에게 푸시 알림 전송"""
