...
class StreamingFileStorage(FileContentService):
    ...
    async def get_file_content(self, file_id: str, token: str) -> Optional[bytes]:
        ...
        try:
            ...
            if file_size > 50 * 1024 * 1024:  # 50MB limit for direct read
                logger.warning(f"File too large for direct read: {file_id} ({file_size} bytes)")
                return None
    ...
    async def save_file_stream(
        ...
        try:
            async with aiofiles.open(temp_path, 'wb') as f:
                async for chunk in content_stream:
                    ...
                    if total_size + len(chunk) > self.max_file_size:
                        raise ValueError(f"File size exceeded: {total_size + len(chunk)}")
            ...
            logger.info(f"Saved file {file_id}: {total_size} bytes, checksum: {checksum.hexdigest()}")
            return True
