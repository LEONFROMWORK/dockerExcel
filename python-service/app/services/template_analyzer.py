...
class TemplateAnalyzer:
    ...
    async def analyze_template_file(
        ...
        try:
            ...
            analysis_result["overall_structure"] = self._extract_overall_structure(
                workbook
            )
            ...
            analysis_result["formula_patterns"] = self._extract_formula_patterns(
                workbook
            )
            ...
            analysis_result["conditional_formatting"] = (
                self._extract_conditional_formatting(workbook)
            )
    ...
    async def _analyze_sheet(self, sheet) -> Dict[str, Any]:
        ...
        max_col_letter = (
            get_column_letter(sheet.max_column) if sheet.max_column else "A"
        )
        ...
        analysis["content_sections"] = self._identify_content_sections(
            content_map, sheet
        )

        # Identify table structures
        analysis["table_structures"] = self._identify_table_structures(
            content_map, sheet
        )

        # Identify input and output areas
        analysis["input_areas"], analysis["output_areas"] = (
            self._identify_input_output_areas(content_map, sheet)
        )
    ...
    def _extract_cell_formatting(self, cell) -> Dict[str, Any]:
        ...
        if cell.border:
            formatting["border"] = {
                ...
                "bottom": (
                    bool(cell.border.bottom.style) if cell.border.bottom else False
                ),
    ...
    def _identify_content_sections(self, content_map: Dict, sheet) -> List[Dict]:
        ...
        for coord, cell_info in content_map.items():
            if (
                cell_info["formatting"].get("font", {}).get("bold")
                or cell_info["formatting"].get("font", {}).get("size", 0) > 12
            ):
                headers.append(
                    {"coordinate": coord, "text": cell_info["value"], "type": "header"}
                )
        ...
        for header in headers:
            section = {
                ...
                "type": "section",
                "estimated_range": self._estimate_section_range(
                    header["coordinate"], sheet
                ),
            }
    ...
    def _identify_input_output_areas(
        self, content_map: Dict, sheet
        ...
        for coord, cell_info in content_map.items():
            ...
            elif cell_info["data_type"] in ["number", "text"] and not cell_info[
                "formatting"
            ].get("font", {}).get("bold"):
                input_areas.append(
                    {
    ...
    def _categorize_formula(self, formula: str) -> str:
        ...
        if any(func in formula_upper for func in ["SUM", "SUMIF", "SUMIFS"]):
            ...
            func in formula_upper for func in ["VLOOKUP", "HLOOKUP", "INDEX", "MATCH"]
        ):
            return "lookup"
        elif any(func in formula_upper for func in ["IF", "AND", "OR", "NOT"]):
            return "logical"
        elif any(
            func in formula_upper for func in ["DATE", "TODAY", "NOW", "YEAR", "MONTH"]
        ):
            return "date_time"
        ...
        for sheet in workbook.worksheets:
            for chart in sheet._charts:
                chart_info = {
                    ...
                    "position": {
                        "anchor": (
                            str(chart.anchor) if hasattr(chart, "anchor") else None
                        )
                    },
                }
            ...
            if hasattr(sheet, "data_validations"):
                for dv in sheet.data_validations.dataValidation:
                    validation_info = {
                        ...
                        "error_message": (
                            dv.errorTitle if hasattr(dv, "errorTitle") else None
                        ),
                    }
    ...
    async def copy_template_to_storage(self, source_path: str, template_id: str) -> str:
        ...
        dest_path = (
            Path(self.template_storage_path) / f"{template_id}{source_path.suffix}"
        )
                    ...
                    templates.append(
                        {
                            ...
                            "complexity_score": metadata["overall_structure"][
                                "complexity_score"
                            ],
                            "sheet_count": metadata["overall_structure"][
                                "total_sheets"
                            ],
