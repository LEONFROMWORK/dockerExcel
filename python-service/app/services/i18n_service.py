...
class I18nService:
    ...
    def localize_template_metadata(
        ...
        if "templates" in localized_data:
            for template_id, template_info in localized_data["templates"].items():
                ...
                complexity_level = complexity_mapping.get(complexity_score, "intermediate")
                complexity_key = f"templates.complexity.{complexity_level}"
                template_info["localized_complexity"] = self.get_text(complexity_key, language)
    ...
    def format_progress_message(
        ...
        """진행률 메시지 포맷팅"""
        messages = {
            "percentage": self.get_text("progress.percentage", language, percent=percent)
        }
