...
def configure_structured_logging(
    log_level: str = "INFO", json_logs: bool = True, service_name: str = "wopi-service"
...
class WOPILogger:
    ...
    def bind_user(self, user_id: str, user_name: str = None):
        """Bind user context."""
        return self.logger.bind(user_id=user_id, user_name=user_name or "unknown")

    def bind_file(self, file_id: str, file_name: str = None):
        """Bind file context."""
        return self.logger.bind(file_id=file_id, file_name=file_name)

    def log_token_generated(
        self, user_id: str, file_id: str, permission: str, token_id: str = None
    ):
    ...
    def log_token_validated(self, token_id: str, valid: bool, reason: str = None):
...
class LoggingMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        try:
            ...
            self.logger.bind_request(
                request_id=request_id, method=scope["method"], path=scope["path"]
            ).error("request.failed", error=str(e), duration_ms=duration_ms)
            raise
