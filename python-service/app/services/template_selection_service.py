...
class TemplateSelectionService:
    ...
    def _load_templates_metadata(self) -> Dict[str, Any]:
        ...
        try:
            metadata_path = os.path.join(
                os.path.dirname(__file__), "../templates/excel/metadata.json"
            )
    ...
    def _load_i18n_metadata(self) -> Dict[str, Any]:
        ...
        try:
            i18n_metadata_path = os.path.join(
                os.path.dirname(__file__), "../templates/excel/metadata_i18n.json"
            )
    ...
    def _initialize_template_vectors(self):
        ...
        try:
            ...
            if template_texts:
                ...
                logger.info(
                    f"템플릿 벡터 초기화 완료: {len(self.template_ids)}개 템플릿"
                )
            else:
    ...
    async def recommend_templates(
        ...
        try:
            ...
            scored_templates = self._calculate_template_scores(
                intent_analysis, data_analysis, user_history, user_tier
            )
    ...
    def _classify_intent_category(self, user_intent: str) -> Dict[str, float]:
        ...
        self.templates_metadata.get("categories", {})
        category_scores = {}
    ...
    async def _analyze_excel_structure(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 데이터 구조 특성 추출
            structure_analysis = {
                "total_sheets": analysis_result.get("summary", {}).get(
                    "total_sheets", 1
                ),
                "total_columns": 0,
                "total_rows": 0,
            ...
            # 데이터 밀도 계산
            total_cells = (
                structure_analysis["total_columns"] * structure_analysis["total_rows"]
            )
            if total_cells > 0:
                filled_cells = analysis_result.get("summary", {}).get(
    ...
    def _calculate_template_scores(
        ...
        templates = self.templates_metadata.get("templates", {})
        weights = self.templates_metadata.get(
            "matching_weights",
            {
                "text_similarity": 0.4,
                "data_structure": 0.3,
        ...
        for template_id, template in templates.items():
            ...
            data_score = (
                self._calculate_data_structure_score(data_analysis, template)
                if data_analysis
                else 0.5
            )

            history_score = self._calculate_user_history_score(user_history, template)
    ...
    def _calculate_text_similarity_score(
        self, intent_analysis: Dict[str, Any], template: Dict[str, Any]
    ) -> float:
        """텍스트 유사도 점수 계산"""
    ...
    def _calculate_data_structure_score(
        self, data_analysis: Dict[str, Any], template: Dict[str, Any]
    ) -> float:
        """데이터 구조 매칭 점수 계산"""
    ...
    def _calculate_user_history_score(
        self, user_history: Dict[str, Any], template: Dict[str, Any]
    ) -> float:
        """사용자 히스토리 점수 계산"""
    ...
    def _calculate_complexity_match_score(
        self, user_complexity: int, template_complexity: int
    ) -> float:
        """복잡도 매칭 점수 계산"""
    ...
    def _format_recommendations(
        self, scored_templates: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """추천 결과 포맷팅"""
        ...
        try:
            ...
            for category, score in category_scores.items():
                if score > 0:
                    ...
                    # 지수 평활법으로 업데이트
                    user_prefs["preferred_categories"][category] = (
                        0.7 * current_pref + 0.3 * score
                    )
            ...
            user_prefs["complexity_preference"] = (
                0.8 * current_complexity + 0.2 * complexity
            )
    ...
    def get_localized_template_by_id(
        self, template_id: str, language: str = "ko"
    ) -> Dict[str, Any]:
        """언어별 템플릿 상세 정보 조회"""
        ...
        # 번역 정보 추가
        if (
            "translations" in i18n_template
            and language in i18n_template["translations"]
        ):
            ...
            localized_template.update(
                {
                    "localized_name": translation.get(
                        "name", base_template.get("name", "")
                    ),
                    "localized_description": translation.get(
                        "description", base_template.get("description", "")
                    ),
                    "localized_features": translation.get(
                        "features", base_template.get("features", [])
                    ),
            ...
            return {"status": "success", "template": localized_template}
        else:
            # 번역이 없으면 기본 템플릿 반환
            return base_result
        ...
        try:
            ...
            for category_id, category_info in base_categories.items():
                ...
                if category_id in i18n_categories:
                    ...
                    if (
                        "translations" in i18n_category
                        and language in i18n_category["translations"]
                    ):
            ...
            return {"status": "success", "categories": localized_categories}
