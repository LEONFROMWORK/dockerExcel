...
async def get_template_details(template_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve template details"
        )
...
async def analyze_template(template_id: str):
    ...
    try:
        ...
        metadata = await template_analyzer.get_template_metadata(template_id)
        template_name = (
            metadata.get("template_name", template_id) if metadata else template_id
        )
...
async def analyze_template_i18n(template_id: str):
    ...
    try:
        ...
        i18n_requirements = (
            await i18n_template_service.analyze_template_i18n_requirements(metadata)
        )
...
async def generate_translations(
    template_id: str, request: TranslationRequest, background_tasks: BackgroundTasks
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Translation generation failed: {str(e)}"
        )
...
async def get_template_translations(template_id: str, language: str):
    ...
    """
    try:
        translations = await i18n_template_service.get_template_translations(
...
async def update_translation(
    template_id: str, language: str, request: TranslationUpdateRequest
):
    """
...
async def download_template(template_id: str):
    ...
    try:
        ...
        metadata = await template_analyzer.get_template_metadata(template_id)
        original_filename = (
            metadata.get("original_filename", template_file.name)
            if metadata
            else template_file.name
        )
...
async def export_localized_template(template_id: str, language: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to export localized template"
        )


# Background task functions
async def generate_template_translations_background(
    template_id: str, target_languages: List[str], use_ai_enhancement: bool = True
):
...
async def get_template_i18n_status(template_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        requirements_file = (
            i18n_template_service.template_translations_path
            / f"{template_id}_i18n_requirements.json"
        )
        ...
        translations_file = (
            i18n_template_service.template_translations_path
            / f"{template_id}_translations.json"
        )
        ...
        return {
            ...
            "status": (
                "complete"
                if has_translations
                else "pending" if has_requirements else "not_analyzed"
            ),
        }
