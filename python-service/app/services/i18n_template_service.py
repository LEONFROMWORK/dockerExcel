...
class I18nTemplateService:
    ...
    async def analyze_template_i18n_requirements(
        ...
        i18n_requirements = {
            ...
            "content_types": set(),
            "suggested_languages": self._suggest_languages_for_template(
                analysis_result
            ),
        }
        ...
        i18n_requirements.update(
            self._calculate_translation_complexity(i18n_requirements["elements"])
        )
    ...
    async def generate_template_translations(
        ...
        for language in target_languages:
            logger.info(
                f"Generating {language} translations for template {template_id}"
            )

            language_translations = await self._generate_language_translations(
                i18n_requirements["elements"], language, use_ai_enhancement
            )
            ...
            if section.get("title"):
                element = I18nTemplateElement(
                    ...
                    context={
                        ...
                        "template_domain": analysis_result.get(
    ...
    async def _process_overall_i18n_elements(
        self, i18n_elements: Dict[str, List[str]], analysis_result: Dict[str, Any]
    ) -> List[I18nTemplateElement]:
            ...
            for text in texts:
                if self._is_translatable_text(text):
                    element = I18nTemplateElement(
                        ...
                        context={
                            "category": category,
                            "template_context": analysis_result.get(
                                "template_name", ""
                            ),
                            "business_domain": analysis_result.get(
                                "business_domain", "general"
                            ),
                        },
                    )
    ...
    async def _generate_language_translations(
        self, elements: List[Any], target_language: str, use_ai_enhancement: bool = True
    ) -> Dict[str, Any]:
        ...
        for domain, domain_elements in domain_groups.items():
            ...
            if use_ai_enhancement and len(domain_elements) > 5:
                ...
                for element in domain_elements:
                    ...
                    translated_text = self._basic_translate(
    ...
    async def _ai_batch_translate(
        self, elements: List[Any], target_language: str, domain: str
    ) -> Dict[str, Any]:
        """Use AI for contextual batch translation"""
        domain_context = self.domain_contexts.get(
            domain, self.domain_contexts["general"]
        )
    ...
    def _suggest_languages_for_template(
        self, analysis_result: Dict[str, Any]
    ) -> List[str]:
        ...
        if any(
            word in template_name for word in ["cash", "flow", "financial", "budget"]
        ):
    ...
    def _calculate_translation_complexity(
        self, elements: List[I18nTemplateElement]
    ) -> Dict[str, Any]:
    ...
    async def _assess_translation_quality(
        self, translations: Dict[str, Any], language: str
    ) -> Dict[str, Any]:
        ...
        quality_metrics["completeness"] = min(
            100, (len(translations) / max(len(translations), 1)) * 100
        )
    ...
    async def _generate_context_adaptations(
        self, translations: Dict[str, Any], language: str, template_id: str
    ) -> Dict[str, Any]:
    ...
    async def _save_i18n_requirements(
        self, template_id: str, requirements: Dict[str, Any]
    ):
        """Save i18n requirements to file"""
        file_path = (
            self.template_translations_path / f"{template_id}_i18n_requirements.json"
        )

        # Convert sets to lists and dataclass objects to dicts for JSON serialization
        requirements_copy = requirements.copy()
        requirements_copy["business_domains"] = list(
            requirements_copy.get("business_domains", set())
        )
        requirements_copy["content_types"] = list(
            requirements_copy.get("content_types", set())
        )
    ...
    async def _load_i18n_requirements(
    ...
    async def _save_translation_results(
        self, template_id: str, results: Dict[str, Any]
    ):
    ...
    async def get_template_translations(
        self, template_id: str, language: str = None
    ) -> Dict[str, Any]:
    ...
    async def update_template_translation(
        self, template_id: str, language: str, key: str, new_translation: str
    ) -> bool:
