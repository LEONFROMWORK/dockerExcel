...
class StyleCacheManager:
    ...
    def __init__(self):
        ...
        self.global_cache = LRUCache(
            max_size=PerformanceSettings.STYLE_CACHE_SIZE,
            ttl=3600  # 1시간
        )
        ...
        with self._cache_lock:
            if workbook_id not in self.workbook_caches:
                self.workbook_caches[workbook_id] = LRUCache(
                    max_size=1000,  # 워크북당 1000개 스타일
                    ttl=1800  # 30분
                )
            return self.workbook_caches[workbook_id]
    
    def get_style(self, style_hash: str, workbook_id: Optional[str] = None) -> Optional[Any]:
    ...
    def put_style(self, style_hash: str, style_data: Any, workbook_id: Optional[str] = None) -> None:
        """스타일 저장"""
...
class CellDataCacheManager:
    ...
    def __init__(self):
        from app.core.constants import PerformanceSettings
        self.cache = LRUCache(
            max_size=PerformanceSettings.CELL_CACHE_SIZE,
            ttl=1800  # 30분
        )
    ...
    def get_cached_cell_data(self, value: Any, formula: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """캐시된 셀 데이터 조회"""
        cache_key = self.get_cell_key(value, formula)
        return self.cache.get(cache_key)
    
    def cache_cell_data(self, value: Any, cell_data: Dict[str, Any], formula: Optional[str] = None) -> None:
        """셀 데이터 캐싱"""
        cache_key = self.get_cell_key(value, formula)
...
def cached_method(cache_manager: Union[LRUCache, StyleCacheManager], 
