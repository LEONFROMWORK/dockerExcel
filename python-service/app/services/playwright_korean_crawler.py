...
class PlaywrightKoreanCrawler:
    ...
    def __init__(self):
        if not PLAYWRIGHT_AVAILABLE:
            raise ImportError("Playwright가 설치되지 않았습니다. pip install playwright playwright-stealth")
            ...
            while page <= max_pages:
                ...
                if page <= max_pages:
                    try:
                        ...
                        await self.page.goto(next_page_url, wait_until='networkidle', timeout=30000)
    ...
    async def _extract_template_info(self, source_url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            description = await desc_element.inner_text() if desc_element else "설명 없음"
    ...
    async def _download_excel_file(self, template_info: Dict[str, Any]) -> Optional[Path]:
        """Excel 파일 다운로드"""
        try:
            # 카테고리별 디렉토리 생성
            ...
            if not file_id:
                ...
                excel_links = re.findall(r'href="([^"]*dw_form\.php[^"]*)"', page_content)
                if excel_links:
            ...
            try:
                ...
                if await self._verify_excel_file(file_path):
                    logger.info(f"💾 Excel 파일 다운로드 완료: {template_info['file_name']}")
                    return file_path
                else:
                    if file_path.exists():
                ...
                response = await download_page.goto(download_url, wait_until='networkidle')
                ...
                if response and response.status == 200:
                    ...
                    if 'html' not in content_type:
                        ...
                        # 파일 검증
                        if await self._verify_excel_file(file_path):
                            logger.info(f"💾 HTTP 다운로드 완료: {template_info['file_name']}")
                            return file_path
                        else:
                            if file_path.exists():
                                file_path.unlink()
                            logger.warning(f"❌ HTTP 다운로드 파일 형식 오류: {template_info['file_name']}")
                            return None
                    else:
                        logger.warning(f"❌ HTML 응답 반환 (인증 실패): {template_info['file_name']}")
                        return None
                else:
                    logger.warning(f"❌ HTTP 요청 실패: {response.status if response else 'No response'}")
                    return None
    ...
    async def _verify_excel_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                header = f.read(4)
                # Excel 파일 시그니처 확인
                return (header.startswith(b'PK') or  # XLSX (ZIP 기반)
                       header.startswith(b'\xd0\xcf\x11\xe0'))  # XLS (OLE 기반)
    ...
    async def _save_results(self):
        ...
        try:
            ...
            results_file = self.metadata_dir / f"playwright_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
