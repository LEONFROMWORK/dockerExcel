...
class FinanceExcelGenerator(DomainExcelGenerator):
    ...
    async def generate_financial_statements(
        self, company_name: str, period: str, data: Optional[Dict[str, Any]] = None
    ) -> str:
        """재무제표 생성"""
    ...
    async def _create_income_statement(
        self, ws, company_name: str, period: str, data: Optional[Dict[str, Any]]
    ):
        """손익계산서 생성"""
    ...
    async def _create_balance_sheet(
        ...
        for i, (item_name, current, previous, item_type) in enumerate(assets):
            ...
            ws.cell(row=row + i, column=2, value=current).style = self.styles[
                "currency"
            ]
            ws.cell(row=row + i, column=3, value=previous).style = self.styles[
                "currency"
            ]
        ...
        # 부채 및 자본 입력
        for i, (item_name, current, previous, item_type) in enumerate(
            liabilities_equity
        ):
            ws.cell(row=row + i, column=4, value=item_name)
            ws.cell(row=row + i, column=5, value=current).style = self.styles[
                "currency"
            ]
            ws.cell(row=row + i, column=6, value=previous).style = self.styles[
                "currency"
            ]
    ...
    async def _create_cash_flow_statement(
        ...
        for item_name, current, previous, item_type in cash_flows:
            if item_type != "blank":
                ...
                if current != "":
                    ws.cell(row=row, column=2, value=current).style = self.styles[
                        "currency"
                    ]
                    ws.cell(row=row, column=3, value=previous).style = self.styles[
                        "currency"
                    ]
    ...
    async def _create_ratio_analysis(self, ws, data: Optional[Dict[str, Any]]):
        """재무비율 분석"""
    ...
    async def _create_financial_dashboard(self, ws, data: Optional[Dict[str, Any]]):
        """재무 대시보드"""
    ...
    async def _create_employee_roster(self, ws, employee_count: int):
        ...
        for i in range(min(employee_count, 50)):  # 최대 50명까지 샘플
            ...
            ws.cell(
                row=row,
                column=7,
                value=f"010-{np.random.randint(1000, 9999)}-{np.random.randint(1000, 9999)}",
            )
            ...
            ws.cell(
                row=row,
                column=9,
                value=f"서울시 {np.random.choice(['강남구', '서초구', '송파구', '마포구'])}",
            )
            ...
            ws.cell(
                row=row,
                column=11,
                value=np.random.choice(["재직", "재직", "재직", "재직", "휴직"]),
            )
    ...
    async def _create_payroll_sheet(self, ws):
        ...
        for i in range(20):  # 20명 샘플
            ...
            ws.cell(
                row=row,
                column=3,
                value=np.random.choice(["경영지원", "영업", "개발", "마케팅"]),
            )
            ws.cell(row=row, column=4, value=position)

            # 급여 계산
            ws.cell(row=row, column=5, value=base_salary).style = self.styles[
                "currency"
            ]
            ...
            for day_col in range(4, 4 + days_in_month):
                ...
                if date.weekday() >= 5:
                    ...
                    code = np.random.choice(
                        attendance_codes,
                        p=[0.85, 0.02, 0.02, 0.01, 0.01, 0.01, 0.02, 0.02, 0.02, 0.02],
                    )
    ...
    async def _create_leave_management(self, ws):
        ...
        for i in range(20):  # 20명 샘플
            ...
            if annual_leave - used_leave <= 3:
                remaining_cell.fill = PatternFill(
                    start_color="FFC7CE", end_color="FFC7CE", fill_type="solid"
                )
...
class SalesExcelGenerator(DomainExcelGenerator):
    ...
    async def _create_customer_sales_analysis(self, ws):
        ...
        for i in range(20):
            ...
            ws.cell(row=row, column=4, value=current_sales).style = self.styles[
                "currency"
            ]
            ...
            ws.cell(row=row, column=7, value=total_sales).style = self.styles[
                "currency"
            ]
    ...
    async def _create_product_sales_analysis(self, ws):
        ...
        for i, product in enumerate(products):
            ...
            ws.cell(row=row, column=5, value=product["quantity"]).number_format = (
                "#,##0"
            )
            ws.cell(row=row, column=6, value=product["sales"]).style = self.styles[
                "currency"
            ]
            ...
            ws.cell(row=row, column=9, value=product["profit_rate"]).style = (
    ...
    async def _create_sales_performance_sheet(self, ws):
        ...
        for i, person in enumerate(sales_people):
            ...
            ws.cell(row=row, column=4, value=person["target"]).style = self.styles[
                "currency"
            ]
            ws.cell(row=row, column=5, value=person["actual"]).style = self.styles[
                "currency"
            ]
            ...
            if person["achievement"] >= 1.1:
                achievement_cell.fill = PatternFill(
                    start_color="C6EFCE", end_color="C6EFCE", fill_type="solid"
                )
                ...
                achievement_cell.fill = PatternFill(
                    start_color="FFF4E5", end_color="FFF4E5", fill_type="solid"
                )
            else:
                achievement_cell.fill = PatternFill(
                    start_color="FFC7CE", end_color="FFC7CE", fill_type="solid"
            ...
            ws.cell(row=row, column=9, value=person["avg_price"]).style = self.styles[
                "currency"
            ]
            ws.cell(row=row, column=10, value=person["incentive"]).style = self.styles[
                "currency"
            ]
...
class InventoryExcelGenerator(DomainExcelGenerator):
    ...
    async def generate_inventory_system(
        self, company_name: str, data: Optional[Dict[str, Any]] = None
    ) -> str:
        """재고 관리 시스템 생성"""
    ...
    async def _create_inventory_status_sheet(self, ws, data: Optional[Dict[str, Any]]):
        ...
        ws.cell(row=total_row, column=10).value = "=SUM(J4:J8)"
    ...
    async def _create_transaction_history_sheet(
        self, ws, data: Optional[Dict[str, Any]]
    ):
        """입출고 내역 시트"""
        # 제목
        ws.merge_cells("A1:K1")
        ...
        for i in range(30):
            ...
            transaction = [
                ...
                (
                    ...
                    if trans_type == "입고"
                    else f"부서{np.random.randint(1, 6)}"
                ),
                f"담당자{np.random.randint(1, 10)}",
                np.random.choice(
                    ["정기입고", "긴급입고", "판매출고", "내부사용", "반품"]
                ),
            ]
...
class ProjectExcelGenerator(DomainExcelGenerator):
    ...
    async def generate_project_management(
        ...
        await self._create_project_overview_sheet(
            overview, project_name, project_duration, data
        )
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = (
            f"project_management_{project_name.replace(' ', '_')}_{timestamp}.xlsx"
        )
        output_path = os.path.join(tempfile.gettempdir(), filename)
        wb.save(output_path)
    ...
    async def _create_project_overview_sheet(
        self, ws, project_name: str, duration: int, data: Optional[Dict[str, Any]]
    ):
        """프로젝트 개요 시트"""
        # 제목
        ws.merge_cells("A1:F1")
        ...
        project_info = [
            ...
            [
                "종료일(예정)",
                (datetime.now() + timedelta(days=duration)).strftime("%Y-%m-%d"),
            ],
        ...
        goals = data.get(
            "goals",
            ["고품질 제품 개발", "일정 내 완료", "예산 내 실행", "고객 만족도 향상"],
        )
        ...
        milestones = [
            ...
            [
                "테스트 완료",
                (datetime.now() + timedelta(days=85)).strftime("%Y-%m-%d"),
                "예정",
                0,
            ],
            [
    ...
    async def _create_gantt_chart_sheet(
        self, ws, duration: int, data: Optional[Dict[str, Any]]
    ):
        """간트 차트 시트"""
        # 제목
        ...
        for week in range(weeks):
            ...
            cell = ws.cell(
                row=3, column=col, value=f"W{week+1}\n{date.strftime('%m/%d')}"
            )
            cell.style = self.styles["header"]
        ...
        for row_idx, task in enumerate(tasks, 4):
            ...
            for col_idx in range(len(task_headers)):
                ...
                if col_idx == 4:  # 종료일
                    cell.value = (start_date + timedelta(days=task[4])).strftime(
                        "%Y-%m-%d"
                    )
                elif col_idx == 3:  # 시작일
                    cell.value = (start_date + timedelta(days=task[3])).strftime(
                        "%Y-%m-%d"
                    )
                elif col_idx == 6:  # 진행률
                    cell.style = self.styles["percentage"]
    ...
    async def _create_resource_management_sheet(
        self, ws, data: Optional[Dict[str, Any]]
    ):
        """리소스 관리 시트"""
        # 제목
            ...
            for col_idx, value in enumerate(allocation, 1):
                ...
                if col_idx > 1:
                    ...
                    ws.conditional_formatting.add(
                        f"{get_column_letter(col_idx)}{row_idx}", rule
                    )
    ...
    async def _create_risk_management_sheet(self, ws, data: Optional[Dict[str, Any]]):
        ...
        for i, label in enumerate(prob_labels):
            ws.cell(row=16, column=3 + i, value=label).alignment = Alignment(
                horizontal="center"
            )

        # 영향도 라벨
        for i, label in enumerate(impact_labels):
            ws.cell(row=17 + i, column=2, value=label).alignment = Alignment(
                horizontal="right"
            )

        # 매트릭스 값
        matrix_values = [[3, 6, 9], [2, 4, 6], [1, 2, 3]]  # 높음  # 중  # 낮음
    ...
    async def _create_project_dashboard(
        self, ws, project_name: str, data: Optional[Dict[str, Any]]
    ):
        """프로젝트 대시보드"""
        # 제목
        ws.merge_cells("A1:J1")
        ...
        for i, (kpi_name, value, description) in enumerate(kpis):
            ...
            ws.cell(row=row + 2, column=col, value=description).font = Font(
                size=10, italic=True
            )
...
class DomainSpecificGenerator:
    ...
    async def generate(self, domain: str, template_type: str, **kwargs) -> str:
        """도메인별 Excel 생성"""
        ...
        raise ValueError(
            f"Unsupported template type: {template_type} for domain: {domain}"
        )
