...
class WebSocketManager:
    ...
    async def connect(
        ...
        logger.info(
            f"WebSocket 연결 수락: {connection_id} (user: {user_id}, task: {task_id})"
        )
    ...
    async def broadcast(
        self, message: Dict[str, Any], exclude_connections: List[str] = None
    ) -> int:
        """모든 연결에 메시지 브로드캐스트"""
    ...
    async def handle_message(self, message: Dict[str, Any]):
        ...
        if message_type == "subscribe_task":
            ...
            if task_id:
                ...
                if (
                    self.connection_id
                    not in websocket_manager.task_connections[task_id]
                ):
                    websocket_manager.task_connections[task_id].append(
                        self.connection_id
                    )

                await websocket_manager.send_to_connection(
                    self.connection_id,
                    {"type": "subscription_confirmed", "task_id": task_id},
                )
            ...
            if task_id and task_id in websocket_manager.task_connections:
                if self.connection_id in websocket_manager.task_connections[task_id]:
                    websocket_manager.task_connections[task_id].remove(
                        self.connection_id
                    )

                await websocket_manager.send_to_connection(
                    self.connection_id,
                    {"type": "unsubscription_confirmed", "task_id": task_id},
                )
