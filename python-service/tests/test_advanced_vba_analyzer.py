...
"""
        ...
        option_explicit_errors = [e for e in errors if e.error_type == "missing_option_explicit"]
...
"""
    ...
    def test_security_score_calculation(self, analyzer):
        ...
        errors = [
            VBAError(
                id="1", category=ErrorCategory.SECURITY, severity=ErrorSeverity.CRITICAL,
                line_number=1, module_name="M1", error_type="shell", description="",
                code_snippet="", fix_suggestion="", confidence=0.9, auto_fixable=False
            ),
            VBAError(
                id="2", category=ErrorCategory.SECURITY, severity=ErrorSeverity.HIGH,
                line_number=2, module_name="M1", error_type="file", description="",
                code_snippet="", fix_suggestion="", confidence=0.8, auto_fixable=False
            ),
            VBAError(
                id="3", category=ErrorCategory.PERFORMANCE, severity=ErrorSeverity.LOW,
                line_number=3, module_name="M1", error_type="perf", description="",
                code_snippet="", fix_suggestion="", confidence=0.7, auto_fixable=True
            )
        ]
    ...
    def test_summary_generation(self, analyzer):
        ...
        errors = [
            VBAError(
                id="1", category=ErrorCategory.RUNTIME, severity=ErrorSeverity.CRITICAL,
                line_number=1, module_name="M1", error_type="runtime_91", description="Error 91",
                code_snippet="", fix_suggestion="", confidence=0.9, auto_fixable=False
            ),
            VBAError(
                id="2", category=ErrorCategory.STYLE, severity=ErrorSeverity.LOW,
                line_number=2, module_name="M1", error_type="style", description="Style issue",
                code_snippet="", fix_suggestion="", confidence=0.8, auto_fixable=True
            )
        ]
    ...
    async def test_memory_cleanup(self, analyzer, tmp_path):
        ...
        with patch('app.services.advanced_vba_analyzer.VBA_Parser') as mock_parser_class:
            mock_parser = MagicMock()
            mock_parser_class.return_value = mock_parser
            ...
            modules = await analyzer._extract_vba_modules(str(test_file))
